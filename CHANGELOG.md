...
- `wheels mcp setup` now writes a stdio-based `.opencode.json` instead of one pointing at the deprecated HTTP MCP endpoint. The two OpenCode templates the setup command reads from (`cli/src/templates/OpenCodeConfig.json` and `app/snippets/OpenCodeConfig.json`) still carried the pre-4.0 shape — `"url": "http://localhost:{PORT}/wheels/mcp", "type": "remote"` — with `{PORT}` left as an unsubstituted literal string. OpenCode users running `wheels mcp setup` ended up with a config trying to connect to a host called `{PORT}` against an endpoint that emits a deprecation warning on every call. Both templates now use the same stdio form already shipped in `tools/build/base/.opencode.json`: `"type": "local", "command": ["wheels", "mcp", "wheels"]`. The CHANGELOG entry from when the stdio shift originally landed claimed all template copies had been updated; this closes the two that were missed (#2735)
