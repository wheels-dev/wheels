...
- `wheels.Seeder.runSeeds()` now treats any `seedOnce()` entry that records `action="failed"` (validation failure on save) as a failure of the whole run: the wrapping transaction is rolled back, `success=false` is returned, the failing model names are included in `message`, and `totalFailed` is exposed alongside `totalCreated` / `totalSkipped`. The CLI bridge (`vendor/wheels/public/views/cli.cfm::runDbSeed`) bubbles `totalFailed` into the JSON envelope so `wheels seed` exits non-zero via the existing `parseCliResponse` → `Wheels.Cli.CommandFailed` path. Previously the framework counted only `totalCreated` and `totalSkipped`, committed the transaction around the failed entries, and returned `success=true` with "Seeding complete." — a half-applied seed file looked identical to a fully-applied one in CI and in `wheels seed` output. seedOnce() remains idempotent, so the caller can fix the offending entry and re-run safely.
