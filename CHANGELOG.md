...
- `wheels new`, `wheels start`, and most other CLI subcommands no longer crash on Windows with `lucee.runtime.exp.NativeException: there is no Resource provider available with the name [c]`. `Module.init()` called `resolveProjectRoot(arguments.cwd)`, which fed `java.io.File.getCanonicalPath()` output (`C:\Users\tim\Projects`) plus a `/vendor/wheels` suffix to Lucee's Resource API. The resulting mixed-slash string was parsed as a URI with scheme `c`, which is not a registered Resource provider, so Lucee bailed before the subcommand could run. `Module.init()` now normalises `variables.cwd`, `variables.moduleRoot`, every interim `candidate` inside `resolveProjectRoot()`, and the final `targetDir` inside `scaffoldNewApp()` to forward slashes via a new bootstrap-safe `$normalizePath()` helper (mirrored as the public `Helpers.normalizePath()` for test coverage). The normalisation is a no-op on POSIX systems and collapses doubled slashes that arise from naïve concatenation while preserving a leading `//` UNC / network-share prefix. `wheels --version` continued to work pre-fix because LuCLI handles `--version` upstream of module dispatch (#2841)
