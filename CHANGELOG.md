...
- `databaseAdapters.CockroachDB.CockroachDBModel` now overrides `$supportsAdvisoryLocks()` to return `false`, matching the H2 / SQL Server treatment from #2665. The CockroachDB adapter extends `PostgreSQLModel`, which reports `true` because Postgres implements `pg_advisory_lock` natively; CockroachDB does not, so its existing `$acquireAdvisoryLock` / `$releaseAdvisoryLock` throw `Wheels.AdvisoryLockNotSupported`. Without the capability override, the four standalone advisory-lock specs in `vendor/wheels/tests/specs/model/lockingSpec.cfc` skipped past their `beforeEach` skip-guard and surfaced as errors on every Cockroach-backed compat-matrix run (Lucee 6, Lucee 7, BoxLang). With this override they skip cleanly, in line with how H2 and SQL Server already behave. No application-facing API change — capability-aware callers that already consult `$supportsAdvisoryLocks()` simply get the correct answer for CockroachDB now
