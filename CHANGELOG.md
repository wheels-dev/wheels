...
- Dev-UI hardening leftovers from #2909 / #2900: the `/wheels/info` JSON branch (`vendor/wheels/public/views/info.cfm`) no longer serializes the full `getApplicationMetadata()` payload — datasource definitions (with credentials) and broader application config flowed through unredacted, bypassing the per-setting redaction the surrounding loops already use. The JSON `application.metadata` field is now whitelisted to `{ mappings }`, mirroring the HTML branch's Mappings table. Separately, `vendor/wheels/public/docs/core.cfm` now validates the `format` request parameter through the new `wheels.Public::$resolveDocFormat()` helper (`^[A-Za-z0-9]+$`, `html` fallback) before interpolating it into the `include "layouts/<format>.cfm"` path — same LFI traversal class `$getRequestFormat` was hardened against. Both surfaces remain dev-environment-gated, so this is defense-in-depth (#2974).
