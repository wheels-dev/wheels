...
- `wheels.middleware.RateLimiter`'s memory-storage path is now safe under concurrent eviction and bounded on the request thread. The emergency-eviction trigger in `handle()` previously called `$evictOldest()` without a lock, so concurrent requests at `maxStoreSize` could all enter eviction at once and each remove 25% of the store, leaving the store far below the target headroom; the trigger now uses the same `wheels-ratelimit-cleanup` named lock as `$maybeCleanup` with double-checked-locking (re-checks `store.size() >= maxStoreSize` inside the lock so secondary entrants no-op when the first eviction has already restored capacity). `$evictOldest`'s second pass previously built an entries-with-ages array over the entire post-expiry store and ran an `ArraySort` closure-comparator across all N keys on the request thread — replaced with bounded-sample LRU: each round samples up to `evictionSampleSize` (default 64) random keys, sorts that small array, evicts the oldest from the sample, and repeats until the eviction target is met. Per-round cost is O(K log K) with K bounded, replacing the previous unbounded O(N log N). `$maybeCleanup`'s full-keyspace scan is also capped at `maxKeysScannedPerCleanup` (default 5000) per call, so the unlucky request that triggers cleanup on a 100k-key store no longer pays for the entire scan; ConcurrentHashMap's unordered iteration means successive cleanups cover different slices over time. Both new knobs are validated at construction (`Wheels.RateLimiter.InvalidConfiguration` for zero/negative values). Database storage was already hardened in #2913; this brings the memory path to parity (#2971).
