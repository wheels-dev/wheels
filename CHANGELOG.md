...
# [4.1.0](https://github.com/wheels-dev/wheels/releases/tag/v4.1.0) => 2026-09-17
...
- Legacy plugin lifecycle hooks (`onPluginLoad` / `onPluginActivate`) now run with per-plugin error isolation: a throwing hook is logged to the `wheels` log and skipped instead of aborting the whole load/activate chain, so one broken legacy plugin can no longer prevent sibling plugins from loading at app startup. This aligns the deprecated-but-supported plugin surface with the per-provider isolation the package system's ServiceProvider `register()`/`boot()` phases received in #2912 (#2975)
...
- The `/wheels/core/tests` and built-in app-test (`app-runner.cfm`) JSON runners now surface scope resolution instead of silently reporting green for the wrong scope: the payload carries `directoryRequested`, `directoryResolved`, `directoryRejected`, `bundlesDiscovered`, and a `warnings[]` array, so a `directory=` value that fails the allowlist (silently swapped for the full default suite) and an allowlist-passing single-spec-file value that discovers 0 bundles are both detectable from the response rather than masquerading as a passing run; `tools/ci/run-tests.sh` and `tools/test-local.sh` consume the new signals and fail loudly on a rejected `directory=` or a 0-bundle discovery (#3083)
...
- `$isSafeRedirectUrl()` now normalizes input per WHATWG URL parsing before classification — embedded ASCII tab/CR/LF are stripped and leading/trailing ASCII whitespace is trimmed — and any URL retaining a C0 control character (`Chr(0)`-`Chr(31)`, `Chr(127)`) is rejected outright. Browsers apply this normalization before navigation, so a URL like `<TAB>//evil.com` or `/<LF>/evil.com` was classified as a same-origin relative path by the pre-strip gate but navigated to `//evil.com` once the browser stripped the whitespace, smuggling an open redirect past `redirectTo()`'s default `allowExternalRedirects=false` policy. Mirrors `#2898`'s strip-then-classify shape and runs ahead of the backslash check so post-strip backslashes are still caught. Deferred-scope finding from `#2898`; specs mirror the prior-art Chr(9)/Chr(10)/Chr(13) test-vector style
...
- The model SQL layer no longer instantiates `wheels.migrator.Migration` on every WHERE build just to read the dialect name — the new `$dialectName()` strips the `Model` suffix from the adapter name now persisted per model class at `$assignAdapter()` time, which also fixes the dialect being probed from the app default datasource for models on a custom datasource (the global `adapterName` setting is rewritten by every model class init, so it was never safe to read per model) ([#2952](https://github.com/wheels-dev/wheels/issues/2952))
