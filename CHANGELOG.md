...
- `Global.cfc::$header()` now copies the `arguments` scope into a plain struct before calling `cfheader(attributeCollection=...)`, so Adobe CF 2023 no longer rejects the call with `Failed to add HTML header` on every request. Adobe 2023 is stricter than Lucee/BoxLang/Adobe 2021 about the shape passed to `attributeCollection` — the raw `arguments` scope is no longer accepted — which prevented any test request from booting past application init. The helper sits on the dispatch path (response content-type/status setup, error handlers), so every controller action went through it: the compat-matrix Adobe 2023 job recorded `0 pass / 0 fail / 0 err` because no test endpoint completed its request. The existing `statusText` strip (added for Adobe CF 2025) was already doing the copy on one branch — this collapses the two branches into a single unconditional copy so every call hands `cfheader` a plain struct. Regression coverage in `vendor/wheels/tests/specs/global/headerSpec.cfc` (#2741)
