...
- `$nonCapturingConstraint()` in `vendor/wheels/Mapper.cfc` now normalizes Java named capture groups (`(?<name>...)`) inside route constraints to non-capturing groups (`(?:...)`), closing the last shape that could silently shift route-variable positions through positional extraction in `$mergeRoutePattern`. The previous fix (#2944) bounded the extraction loop so an extra group could no longer crash param assembly, but a named group inside a constraint was still position-shifting because the scanner skipped every `(?...)` flavor without distinguishing the named-capturing form from lookbehinds. The scanner now disambiguates: `(?<=...)` and `(?<!...)` (lookbehinds — non-capturing) are left untouched, while `(?<name>...)` consumes the `<name>` token and emits `(?:` in its place, since positional extraction does not consult group names. Bare `(...)`, explicit `(?:...)`, lookaheads, and parens inside character classes continue to behave as before. Covered by `vendor/wheels/tests/specs/mapper/NonCapturingConstraintSpec.cfc` (#2976).
