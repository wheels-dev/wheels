...
- Legacy plugin lifecycle hooks `$invokeOnPluginLoad` and `$invokeOnPluginActivate` now wrap each plugin's hook invocation in its own try/catch, mirroring the ServiceProvider isolation landed in #2912. Pre-fix, a single throwing legacy plugin took down the entire load or activate chain — every plugin sorted after the failing one was silently skipped, and app startup either aborted or proceeded with a partially-initialized plugin set depending on where the throw landed. The failure is now logged via `WriteLog(type="error", file="wheels")` and sibling plugins still receive their hooks. `onPluginLoad`'s post-hook application-scope sync is skipped when the hook throws so a partially-mutated context struct doesn't leak back into the live application scope. Log-and-skip only — no registry mutation, since later phases don't iterate the plugin list for the legacy non-ServiceProvider surface (#2975).
