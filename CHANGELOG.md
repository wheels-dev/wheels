...
- `wheels.middleware.RateLimiter` now validates `windowSeconds > 0` and `maxRequests >= 0` at construction time, throwing `Wheels.RateLimiter.InvalidConfiguration` with a parameter-named message instead of leaking a generic `You cannot divide by zero.` from the `fixedWindow` / `tokenBucket` strategy math. `maxRequests = 0` remains a legitimate kill-switch value (block every request)
