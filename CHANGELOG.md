...
- `env("KEY")` no longer throws `UndefinedElementException: Element DEFAULT is undefined in ARGUMENTS` on Adobe CF 2023/2025 when called with a single positional argument (the common "must-be-set" case). `default` is a CFML reserved word (switch/case/default) and the Adobe CF argument binder skips seeding the scope key when the caller omits the matching argument — so `arguments.default` is undefined despite the signature default `""`. Lucee and BoxLang seed it correctly, so the bug was Adobe-only and only surfaced once the dispatch + test-runner layers in this PR (`$header()`, BaseReporter `reset()`, `runner.cfm` migration) stopped the `cfheader` cascade from masking real test exceptions. `envHelperSpec.cfc:28` already exercises the path; the failing assertion's `UndefinedElementException` was escaping TestBox's per-spec catch and bringing down the entire Adobe leg of the compat matrix. Fix uses defensive `StructKeyExists(arguments, "default") ? arguments.default : ""` access — public `@default` API is unchanged, callers that supply the second arg are unaffected (#2756)
- `$content()` in `vendor/wheels/Global.cfc` and the bare `cfheader`/`cfcontent` calls in `vendor/wheels/tests/runner.cfm` now defer to the defensive `application.wo.$header()` / `application.wo.$content()` helpers so the test-runner response setup degrades gracefully when the response is already committed. After the BaseReporter `reset()` fix above let TestBox produce its JSON report, the runner's post-test `cfcontent(type="application/json")` / `cfheader(name="Access-Control-Allow-Origin", value="*")` calls at `runner.cfm:159-160` started throwing `InvalidHeaderException: Failed to add HTML header` on Adobe CF 2023/2025 — by that point the response buffer has flushed mid-`testBox.run()` (any test output crossing the engine's buffer threshold commits the response), so the headers can't be modified. `$content()` picks up the same `$responseCommitted()` short-circuit as `$header()`, and the six `cfheader` / four `cfcontent` sites in `runner.cfm` now route through the framework helpers. The status-code header is the signal CI parsers key on, so best-effort is the right contract — a committed response keeps whatever statuscode the engine already wrote, and the JSON body still appends. Third and final layer of the adobe2023/adobe2025 compat-matrix unwind, on top of #2756 (dispatch `$header()`) and the BaseReporter `reset()` fix above (#2756)
