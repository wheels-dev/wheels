...
- `wheels migrate latest` no longer crashes on PostgreSQL (and CockroachDB) when a migration emits an inline foreign-key constraint — e.g. any `wheels generate scaffold ... --belongsTo=author`. `wheels.databaseAdapters.PostgreSQL.PostgreSQLMigrator` was missing the `addForeignKeyOptions(sql, options)` method that `Abstract.createTable()` invokes for every inline foreign key (via `ForeignKeyDefinition.toForeignKeySQL()` → `adapter.addForeignKeyOptions()`). Every other adapter (MySQL, SQLite, Microsoft SQL Server, Oracle) implements it, so only PostgreSQL- and CockroachDB-backed apps threw `Component [wheels.databaseAdapters.PostgreSQL.PostgreSQLMigrator] has no function with name [addForeignKeyOptions]` at runtime. The new implementation emits standard table-level `FOREIGN KEY (col) REFERENCES tbl (refCol)` DDL — the same shape MySQL and Microsoft SQL Server use, which PostgreSQL accepts — but routes the three identifiers through the adapter's own `quoteColumnName` / `quoteTableName` so the references honour the `migratorObjectCase` setting and match how the columns are declared in the same `CREATE TABLE` (PostgreSQL folds unquoted identifiers to lower case). `CockroachDBMigrator` extends `PostgreSQLMigrator` and inherits the fix. The reporter's "fails on Linux, works on Windows" note was a datasource red herring: the `wheels new` default is SQLite (which already has the method), so only a PostgreSQL/CockroachDB datasource ever reached the gap — the bug is OS-independent and adapter-specific. Regression coverage in `vendor/wheels/tests/specs/migrator/postgreSQLForeignKeyOptionsSpec.cfc` instantiates the PostgreSQL and CockroachDB adapters directly (so it exercises the fix on every engine × DB, including the default SQLite local loop) and drives the exact failing `ForeignKeyDefinition.toForeignKeySQL()` path from the issue (#2876)
