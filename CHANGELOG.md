...
- `QueryBuilder.whereIn()` / `whereNotIn()` with an empty array no longer emit malformed SQL (`property IN ()`). Previously, passing an empty list or array to either method produced syntactically invalid SQL that surfaced as a generic JDBC syntax error from the database, with no pointer back to the call site that built the empty collection. The methods now short-circuit to the SQL-spec-correct shapes: `whereIn(prop, [])` appends `1 = 0` (no rows match) and `whereNotIn(prop, [])` appends `1 = 1` (every row matches), composing cleanly with other clauses in the chain (`.where("status", "active").whereIn("id", []).count()` returns 0). Matches the behaviour every mature ORM converged on (Rails, Sequel, Django, Laravel Eloquent). Four new specs in `vendor/wheels/tests/specs/model/queryBuilderSpec.cfc` cover empty-array, empty-list, composition-with-other-clauses, and the `whereNotIn` mirror case. Both copies of the query-builder guide were updated to document the short-circuit in the methods table (#2736)
