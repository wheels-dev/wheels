---
title: hasMany()
description: "Sets up a one-to-many association between the current model and another model. This allows you to easily fetch, join, and manage related records in a relational"
sidebar:
  label: hasMany()
  order: 0
---
...
Sets up a one-to-many association between the current model and another model. This allows you to easily fetch, join, and manage related records in a relational way while following Wheels conventions.
...
<pre><code class='javascript'>1. Specify that instances of this model has many comments (the table for the associated model, not the current, should have the foreign key set on it).
hasMany(&quot;comments&quot;);
