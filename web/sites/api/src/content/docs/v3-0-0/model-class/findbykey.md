---
title: findByKey()
description: "The findByKey() function retrieves a single record from the database using its primary key value and returns it as an object by default. If the record is not fo"
sidebar:
  label: findByKey()
  order: 0
---
...
| `returnAs` | `string` | no | `object` | Set to `objects` to return an array of objects, set to `structs` to return a struct of structs, set to `array` to return an array of structs, set to `query` to return a query result set, or set to 'sql' to return the executed SQL query as a string. |
...
<pre><code class='javascript'>1. Getting the author with the primary key value `99` as an object
