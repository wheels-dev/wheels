---
title: update()
description: "Updates an existing model object with the supplied properties and saves the changes to the database. It returns true if the save was successful and false otherw"
sidebar:
  label: update()
  order: 0
---
...
| `allowExplicitTimestamps` | `boolean` | no | `false` | Set this to `true` to allow explicit assignment of `updatedAt` property |

## Examples

<pre><code class='javascript'>1. Get a post object and then update its title in the database
