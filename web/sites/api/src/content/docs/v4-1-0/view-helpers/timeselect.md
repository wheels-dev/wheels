---
title: timeSelect()
description: "Builds and returns a string containing three <code>select</code> form controls for hour, minute, and second based on the supplied objectName and property."
sidebar:
  label: timeSelect()
  order: 0
---
...
Builds and returns a string containing three <code>select</code> form controls for hour, minute, and second based on the supplied objectName and property.
...
<pre><code class='javascript'>&lt;!--- Basic time select bound to a model object (hour, minute, second) ---&gt;
