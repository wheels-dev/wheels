...
description: "Validates that the value of the specified property matches the length requirements supplied. Use the exactly, maximum, minimum and within arguments to specify t"
sidebar:
...
| `message` | `string` | yes | `[property] is the wrong length` | Supply a custom error message here to override the built-in one. |
...
| `exactly` | `numeric` | yes | `0` | The exact length that the property value must be. |
| `maximum` | `numeric` | yes | `0` | The maximum length that the property value can be. |
| `minimum` | `numeric` | yes | `0` | The minimum length that the property value can be. |
