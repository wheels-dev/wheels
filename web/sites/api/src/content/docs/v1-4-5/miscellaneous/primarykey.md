---
title: primaryKey()
description: "Returns the name of the primary key for this model's table. This is determined through database introspection. If composite primary keys have been used, they wi"
sidebar:
  label: primaryKey()
  order: 0
---

## Signature

`primaryKey()` — returns `any`




## Description

Returns the name of the primary key for this model's table. This is determined through database introspection. If composite primary keys have been used, they will both be returned in a list. This function is also aliased as primaryKeys().

## Parameters

<div class="wd-params-table">

| Name | Type | Required | Default | Description |
| ---- | ---- | -------- | ------- | ----------- |
| `position` | `numeric` | yes | `0` | If you are accessing a composite primary key, pass the position of a single key to fetch. |

</div>

## Examples

<pre>primaryKey([ position ]) &lt;!--- Get the name of the primary key of the table mapped to the `employee` model (which is the `employees` table by default) ---&gt;
&lt;cfset keyName = model(&quot;employee&quot;).primaryKey()&gt;</pre>
