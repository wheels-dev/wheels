...
Note that the above conventions are for `GET` requests and only apply when you have a `wildcard()` call in `config/routes.cfm` (which is the default). See [Routing](/2.5.0/guides/handling-requests-with-controllers/routing) for instructions on overriding this behavior and how to deal with `PUT`, `POST` etc.
...
See [Routing](/2.5.0/guides/handling-requests-with-controllers/routing) for instructions on overriding this behavior.
...
For information on overriding this behavior, refer to documentation for the [renderView()](https://wheels.dev/api/v2.5.0/controller.renderview.html) function and read the [Pages](/2.5.0/guides/displaying-views-to-users/pages) chapter.
...
For information on overriding the layout file to be loaded by an action, see the chapter on [Layouts](/2.5.0/guides/displaying-views-to-users/layouts)] and documentation for the [renderView](https://wheels.dev/api/v2.5.0/controller.renderview.html) function.
...
Refer to the [Configuration and Defaults](/2.5.0/guides/working-with-cfwheels/configuration-and-defaults) chapter for instructions on overriding data source information.
...
For instructions on overriding database naming conventions, refer to documentation for the [table()](https://wheels.dev/api/v2.5.0/model.table.html) function and the chapter on [Object Relational Mapping](/2.5.0/guides/database-interaction-through-models/object-relational-mapping).
...
For information on overriding column and property names, refer to documentation for the [property()](https://wheels.dev/api/v2.5.0/model.property.html) function and the [Object Relational Mapping](/2.5.0/guides/database-interaction-through-models/object-relational-mapping) chapter.
...
For more details on what you can configure, read the [Configuration and Defaults](/2.5.0/guides/working-with-cfwheels/configuration-and-defaults) chapter.
