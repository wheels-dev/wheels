---
title: "Tutorial: CFWheels, AJAX, and You"
description: >-
...
But let's make it into an asynchronous request. Add this JavaScript (either on the page inside `script` tags or in a separate `.js` file included via [javaScriptIncludeTag()](https://wheels.dev/api/v2.5.0/controller.javaScriptIncludeTag.html) ):
...
The last thing that we need to do is implement the `say/hello` action. Note that the request expects a `dataType` of `JSON`. By default, CFWheels controllers only generate HTML responses, but there is an easy way to generate JSON instead using CFWheels's [provides()](https://wheels.dev/api/v2.5.0/controller.provides.html) and [renderWith()](https://wheels.dev/api/v2.5.0/controller.renderWith.html) functions:
