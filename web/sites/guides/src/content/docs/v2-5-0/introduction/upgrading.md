...
The [addRoute()](https://wheels.dev/api/v2.5.0/addroute.html) function has been removed in CFWheels 2.0 in favor of a new routing API. See the [Routing](/2.5.0/guides/handling-requests-with-controllers/routing) chapter for information about the new RESTful routing system.
...
1. In `controllers/Controller.cfc`, add a call to [protectsFromForgery()](https://wheels.dev/api/v2.5.0/protectsfromforgery.html) to the `config` method.
2. Add a call to the [csrfMetaTags()](https://wheels.dev/api/v2.5.0/controller.csrfMetaTags.html) helper in your layouts' `<head>` sections.
3. Configure any AJAX calls that `POST` data to your application to pass the `authenticityToken` from the `<meta>`tags generated by [csrfMetaTags()](https://wheels.dev/api/v2.5.0/controller.csrfMetaTags.html) as an `X-CSRF-TOKEN` HTTP header.
