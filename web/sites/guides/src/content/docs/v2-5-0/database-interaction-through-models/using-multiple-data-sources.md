...
Wheels has built-in functionality for this so that you don't have to revert back to writing the queries and setting the data source manually whenever you need to use a data source other than the default one. In order accomplish this, you will use the [dataSource()](https://wheels.dev/api/v2.5.0/model.datasource.html) function.

### Using the dataSource() Function

Overriding the default data source is done on a per model basis in Wheels by calling the [dataSource()](https://wheels.dev/api/v2.5.0/model.datasource.html)function from within your model's `config()` method. By doing this, you instruct wheels to use that data source whenever it interacts with that model.
...
```javascript title="models/Photo.cfc"
component extends="Model" {
...
```javascript title="models/PhotoGallery.cfc"
component extends="Model" {
...
Because the `photo` model is the main model being used in the following example, its data source (`myFirstDatabase`) will be the one used in the query that [findAll()](https://wheels.dev/api/v2.5.0/model.findall.html) ends up executing.
