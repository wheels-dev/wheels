...
Here is a list of values that you can grab from [mimeTypes()](https://wheels.dev/api/v2.5.0/controller.mimeTypes.html) with CFWheels out\
of the box.
...
You can use [addFormat()](https://wheels.dev/api/v2.5.0/controller.addformat.html) to set more types to the appropriate MIME type for reference. For example, we could set a Microsoft Word MIME type in\
...
[provides()](https://wheels.dev/api/v2.5.0/controller.provides.html) from within your controller's `config()` method.
...
By calling the [provides()](https://wheels.dev/api/v2.5.0/controller.provides.html) function in `config()`, you are instructing the CFWheels controller to be ready to provide content in a number of formats.\
Possible choices to add to the list are `html` (which runs by default), `xml`,\
`json`, `csv`, `pdf`, and `xls`.

This is coupled with a call to [renderwith()](https://wheels.dev/api/v2.5.0/controller.renderwith.html) in the following actions. In the example above, we are setting a query result of products and passing it\
to [renderwith()](https://wheels.dev/api/v2.5.0/controller.renderwith.html). By passing our data to this function, CFWheels gives us the ability to respond to requests for different formats, and it even gives us the option to just let CFWheels handle the generation of certain formats automatically.

You can also use the [onlyProvides()](https://wheels.dev/api/v2.5.0/controller.onlyProvides.html) call in an individual controller action to define which formats the action will respond with. This can be used to define behavior in individual actions or to override the controller's `config()`.
...
Responding to requests for the HTML version is the same as you're already used to with [Rendering Content](/2.5.0/guides/handling-requests-with-controllers/rendering-content). [renderwith()](https://wheels.dev/api/v2.5.0/controller.renderwith.html) will accept the same arguments as [renderView()](https://wheels.dev/api/v2.5.0/controller.renderview.html), and you create just a view template in the `views` folder like normal.

### Automatic Generation of XML and JSON Formats

If the requested format is `xml` or `json`, the [renderwith()](https://wheels.dev/api/v2.5.0/controller.renderwith.html) function will automatically transform the data that you provide it. If you're fine with what the function produces, then you're done!
...
First of all, always return data as an array of structs. This is done by using the `returnAs` argument (on [findAll()](https://wheels.dev/api/v2.5.0/model.findall.html) for example), like this:
...
Secondly, make use of CFWheels ability to return the JSON values in a specified type. This is done in the [renderWith()](https://wheels.dev/api/v2.5.0/controller.renderwith.html)function, like this:
