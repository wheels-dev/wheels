...
response functions ( [renderView()](https://wheels.dev/api/v2.5.0/v2.2#controller.renderview), [sendFile()](https://wheels.dev/api/v2.5.0/v2.2#controller.sendfile) etc) then Wheels will\
...
chapters about [Redirecting Users](/2.5.0/guides/handling-requests-with-controllers/redirecting-users) and [Sending Files](/2.5.0/guides/handling-requests-with-controllers/sending-files) cover the other two\
response methods.

### Rendering a Page

This is the most common way of responding to the user. It's done with the\
[renderView()](https://wheels.dev/api/v2.5.0/v2.2#controller.renderview) function, but most often you probably won't call it\
...
it's very important to keep in mind that [renderView()](https://wheels.dev/api/v2.5.0/v2.2#controller.renderview) will\
**not run the code** for the controller's action—all it does is process the view\
page for it.

You can also call [renderView()](https://wheels.dev/api/v2.5.0/v2.2#controller.renderview) explicitly if you wish to cache the response\
or use a different layout than the default one.

If the `controller` and `action` arguments do not give you enough flexibility,\
you can use the `template` argument that is available for [renderView()](https://wheels.dev/api/v2.5.0/v2.2#controller.renderview).

Refer to the [Pages](/2.5.0/guides/displaying-views-to-users/pages) chapter for more details about rendering content. More\
specifically, that chapter describes where to place those files and what goes in\
them.

### Rendering a Partial

This is done with the [renderPartial()](https://wheels.dev/api/v2.5.0/controller.renderpartial.html) function. It's most often used with\
...
the [renderNothing()](https://wheels.dev/api/v2.5.0/controller.rendernothing.html) function to tell Wheels to just render an empty page to\
the browser.

### Rendering Text

This is done with the [renderText()](https://wheels.dev/api/v2.5.0/controller.rendertext.html) function. It just returns the text you\
...
handy. It's available on both [renderView()](https://wheels.dev/api/v2.5.0/v2.2/controller.renderView.html) and [renderPartial()](https://wheels.dev/api/v2.5.0/controller.renderpartial.html).\
...
[renderView()](https://wheels.dev/api/v2.5.0/v2.2/controller.renderView.html) and [renderPartial()](https://wheels.dev/api/v2.5.0/controller.renderpartial.html). Just pass in `cache=true` (to use\
...
We cover caching in greater detail in the [Caching](/2.5.0/guides/handling-requests-with-controllers/caching) chapter.

### Using a Layout

The [renderView()](https://wheels.dev/api/v2.5.0/v2.2/controller.renderView.html) function accepts an argument named `layout`. Using this\
...
called [Using Layouts](/2.5.0/guides/displaying-views-to-users/layouts).
