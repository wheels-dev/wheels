...
Middleware lets you intercept and modify requests at the **dispatch level**, before a controller is even instantiated. This is different from [filters](/v3-1-0/handling-requests-with-controllers/using-filters/), which run inside a specific controller.
...
For simple CORS needs, you may prefer the existing [CORS Requests](/v3-1-0/handling-requests-with-controllers/cors-requests/) guide which covers header-only approaches.
...
For the full multi-tenancy guide — including shared models, tenant migrations, and background jobs — see [Multi-Tenancy](/v3-1-0/working-with-wheels/multi-tenancy/).
