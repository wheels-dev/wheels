...
If you're using [multi-tenancy](/v3-1-0/working-with-wheels/multi-tenancy/), background jobs automatically capture the current tenant context when enqueued and restore it before `perform()` runs. No extra code is needed.
