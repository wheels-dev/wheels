...
In Wheels, one way to create objects that represent records in our table is by calling the [new()](https://wheels.dev/api/v3.1.0/model.new.html) class-level method.
...
At this point, the `newAuthor` object only exists in memory. We save it to the database by calling its [save()](https://wheels.dev/api/v3.1.0/model.save.html) method.
...
If you want to create a new object based on parameters sent in from a form request, the [new()](https://wheels.dev/api/v3.1.0/model.new.html) method conveniently accepts a struct as well. As we'll see later, when you use the Wheels form helpers, they automatically turn your form variables into a struct that you can pass into [new()](https://wheels.dev/api/v3.1.0/model.new.html) and other methods.
...
If you want to save a new author to the database right away, you can use the [create()](https://wheels.dev/api/v3.1.0/model.create.html) method instead.
...
Of course, if you do need to access the database default immediately after saving, Wheels allows this. Simply add `reload=true` to the [create()](https://wheels.dev/api/v3.1.0/model.create.html), [update()](https://wheels.dev/api/v3.1.0/model.update.html), or [save()](https://wheels.dev/api/v3.1.0/model.save.html) methods:
...
Sometimes a database default isn't the most appropriate solution because the value is only set after the model has been inserted. If you want to set a default value when it is first created with [new()](https://wheels.dev/api/v3.1.0/model.new.html) or [create()](https://wheels.dev/api/v3.1.0/model.create.html), then you can pass the `defaultValue` argument of the [property()](https://wheels.dev/api/v3.1.0/model.property.html) method used in your model's `config()` block.
