...
If your database table contains a field that is a foreign key to another table, then this is where to use the [belongsTo()](https://wheels.dev/api/v3.1.0/model.belongsto.html) function.
...
The [hasOne()](https://wheels.dev/api/v3.1.0/model.hasone.html) association is not used as often as the [hasMany()](https://wheels.dev/api/v3.1.0/model.hasmany.html) association, but it has its use cases. The most common use case is when you have a large table that you have broken down into two or more smaller tables (a.k.a. denormalization) for performance reasons or otherwise.
...
However, this is not a definite requirement. Wheels associations are completely independent of one another, so it's perfectly OK to setup a [hasMany()](https://wheels.dev/api/v3.1.0/model.hasmany.html) association without specifying the related [belongsTo()](https://wheels.dev/api/v3.1.0/model.belongsto.html) association.
...
To join data from related tables in our [findAll()](https://wheels.dev/api/v3.1.0/model.findall.html) calls, we simply need to use the include argument. Let's say that we wanted to include data about the author in our  [findAll()](https://wheels.dev/api/v3.1.0/model.findall.html) call for `posts`.
...
Let's say that you tell Wheels through a [hasMany()](https://wheels.dev/api/v3.1.0/model.hasmany.html) call that a `post` _has many_ `comments`. What happens then is that Wheels will enrich the post model by adding a bunch of useful methods related to this association.

If you wanted to get all `comments` that have been submitted for a `post`, you can now call `post.comments()`. In the background, Wheels will delegate this to a [findAll()](https://wheels.dev/api/v3.1.0/model.findall.html) call with the `where` argument set to `postid=#post.id#`.
...
Given that you have told Wheels that a `post` _has many_ `comments` through a [hasMany()](https://wheels.dev/api/v3.1.0/model.hasmany.html) call, here are the methods that will be made available to you on the `post` model.
...
The [hasOne()](https://wheels.dev/api/v3.1.0/model.hasone.html) association adds a few methods as well. Most of them are very similar to the ones added by [hasMany()](https://wheels.dev/api/v3.1.0/model.hasmany.html).

Given that you have told Wheels that an `author` _has one_ `profile` through a [hasOne()](https://wheels.dev/api/v3.1.0/model.hasone.html) call, here are the methods that will be made available to you on the `author` model.
...
The [belongsTo()](https://wheels.dev/api/v3.1.0/model.belongsto.html) association adds a couple of methods to your model as well.

Given that you have told Wheels that a `comment` belongs to a `post` through a [belongsTo()](https://wheels.dev/api/v3.1.0/model.belongsto.html) call, here are the methods that will be made available to you on the `comment` model.
...
With the `shortcut` argument to [hasMany()](https://wheels.dev/api/v3.1.0/model.hasmany.html), you can have Wheels create a dynamic method that lets you bypass the join model and instead reference the model on the other end of the many-to-many relationship directly.

For our example above, you can alter the [hasMany()](https://wheels.dev/api/v3.1.0/model.hasmany.html) call on the `customer` model to look like this instead:
...
This functionality relies on having set up all the appropriate [hasMany()](https://wheels.dev/api/v3.1.0/model.hasmany.html) and [belongsTo()](https://wheels.dev/api/v3.1.0/model.belongsto.html) associations in all 3 models (like we have in our example in this chapter).
...
The `through` argument accepts a list of 2 association names. The first argument is the name of the [belongsTo()](https://wheels.dev/api/v3.1.0/model.belongsto.html)association (set in the `subscription` model in this case), and the second argument is the [hasMany()](https://wheels.dev/api/v3.1.0/model.hasmany.html) association going back the other way (set in the `publication` model).
