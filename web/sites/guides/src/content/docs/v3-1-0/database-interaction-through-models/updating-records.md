...
When you have created or retrieved an object, you can save it to the database by calling its [save()](https://wheels.dev/api/v3.1.0/model.save.html) method. This method returns `true` if the object passes all validations and the object was saved to the database. Otherwise, it returns `false`.

This chapter will focus on how to update records. Read the [Creating Records](https://wheels.dev/3.1.0/guides/database-interaction-through-models/creating-records) chapter for more information about how to create new records.
...
To cut down even more on lines of code, you can also combine the reading and saving of the objects by using the class-level methods [updateByKey()](https://wheels.dev/api/v3.1.0/model.updatebykey.html) and [updateAll()](https://wheels.dev/api/v3.1.0/model.updateall.html).

### The updateByKey() Method

Give the [updateByKey()](https://wheels.dev/api/v3.1.0/model.updatebykey.html) method a primary key value (or several if you use composite keys) in the `key` argument, and it will update the corresponding record in your table with the properties you give it. You can pass in the properties either as named arguments or as a struct to the `properties` argument.
...
By default, [updateByKey()](https://wheels.dev/api/v3.1.0/model.updatebykey.html) will fetch the object first and call the `update()` method on it, thus invoking any callbacks and validations you have specified for the model. You can change this behavior by passing in `instantiate=false`. Then it will just update the record from the table using a simple UPDATE query.

An example of using [updateByKey()](https://wheels.dev/api/v3.1.0/model.updatebykey.html) by passing a struct:

```javascript
result = model("post").updateByKey(33, params.post);
```

And an example of using [updateByKey()](https://wheels.dev/api/v3.1.0/model.updatebykey.html) by passing named arguments:
...
The [updateAll()](https://wheels.dev/api/v3.1.0/model.updateall.html) method allows you to update more than one record in a single call. You specify what records to update with the `where` argument and tell Wheels what updates to make using named arguments for the properties.
...
Unlike [updateByKey()](https://wheels.dev/api/v3.1.0/model.updatebykey.html), the [updateAll()](https://wheels.dev/api/v3.1.0/model.updateall.html) method will not instantiate the objects by default. That could be really slow if you wanted to update a lot of records at once.
