...
<Aside type="caution">
  `windowSeconds` must be a positive integer. A value of `0` or less throws `Wheels.RateLimiter.InvalidConfiguration` at app startup — the same place you set your other middleware — so misconfigurations surface immediately rather than causing a cryptic divide-by-zero at runtime. `maxRequests = 0` is a valid kill-switch that blocks every request (useful for maintenance windows or route-scope lockdowns).
</Aside>
