...
For the full argument list, placement semantics, and a run-through of the order of operations, see [Controllers and Actions](/v4-0-1-snapshot/basics/controllers-and-actions/). This page focuses on what to put inside the filter.
...
`params.post` is a model instance populated by route model binding, not a raw struct. Without binding, `params.post` doesn't exist and the dev-mode warning fires telling you to enable it. See [Route Model Binding](/v4-0-1-snapshot/basics/routing/) for the one-line fix (`binding=true` on the resource).
...
On the model side, keep role predicates out of ad-hoc WHERE strings. Use a named scope or the query builder — see [Query Builder and Scopes](/v4-0-1-snapshot/basics/query-builder-and-scopes/) for `model("User").where("role", "admin").get()` and the `scope(name="admins", where="role = 'admin'")` shorthand.
...
<CardGrid>
  ...
  <LinkCard
    ...
    href="/v4-0-1-snapshot/basics/routing/"
  />
  <LinkCard
