...
<Aside type="note">
  You should already know: Wheels controllers, the `params` struct, and form helpers. Finish the [tutorial](/v4-0-1-snapshot/start-here/tutorial/) first. Skim [Forms and Form Helpers](/v4-0-1-snapshot/basics/forms-and-form-helpers/) and [Controllers and Actions](/v4-0-1-snapshot/basics/controllers-and-actions/) if either feels shaky.
</Aside>
...
## Storing binary content in a database column

If your schema keeps file content in a `blob` / `longblob` / `bytea` column instead of on disk, you can assign the raw bytes directly to the model property:

```cfm {test:compile} title="app/controllers/Photos.cfc"
component extends="Controller" {
    function create() {
        local.tmpDir = expandPath("/var/uploads/tmp/");
        cffile(action="upload", fileField="photo[fileData]", destination=local.tmpDir, nameconflict="makeunique");

        local.bytes = FileReadBinary(local.tmpDir & cffile.serverFile);
        fileDelete(local.tmpDir & cffile.serverFile);

        local.photo = model("Photo").new(
            filename = cffile.clientFile,
            fileData = local.bytes
        );

        if (local.photo.save()) {
            redirectTo(route="photo", key=local.photo.id);
        } else {
            renderView(action="new");
        }
    }
...
<Aside type="note">
  `FileReadBinary()` returns a `byte[]` on Lucee 7 and Adobe CF, but a CFML array of integers on BoxLang and some Lucee 6 configurations. Wheels' model property setter accepts both shapes for binary columns — the JDBC driver handles the binding correctly on all engines.
</Aside>
...
- **Rate limit.** Both sides. Upload endpoints invite abuse (disk fill, compute-heavy validation); download endpoints invite scraping. Pair with [Rate Limiting](/v4-0-1-snapshot/digging-deeper/rate-limiting/) per IP and per user.

## Related guides

<CardGrid>
  <LinkCard title="Forms and Form Helpers" href="/v4-0-1-snapshot/basics/forms-and-form-helpers/" description="Where fileField and startFormTag come from." />
  <LinkCard title="Controllers and Actions" href="/v4-0-1-snapshot/basics/controllers-and-actions/" description="Filters, params, and the action lifecycle the upload flow sits inside." />
  <LinkCard title="Background Jobs" href="/v4-0-1-snapshot/digging-deeper/background-jobs/" description="Generate thumbnails, transcode video, or virus-scan uploads on a worker." />
  <LinkCard title="Rate Limiting" href="/v4-0-1-snapshot/digging-deeper/rate-limiting/" description="Throttle both upload and download endpoints per IP and per user." />
</CardGrid>
