...
See [Migrating from Kamal](/v4-0-1-snapshot/deployment/migrating-from-kamal/) for the complete switch-over checklist.
...
```yaml title="Ruby Kamal ERB (NOT supported in wheels deploy)"
...
```yaml title="${VAR} interpolation (supported)"
service: ${APP_NAME}
...
- **Not Kubernetes.** `wheels deploy` drives `docker` remotely over SSH, not the Kubernetes API. For k8s, use your existing pipeline; see [Docker Deployment](/v4-0-1-snapshot/deployment/docker-deployment/) for image hygiene guidance.
- **Not systemd-native.** For servlet-container deploys under systemd (CommandBox, Tomcat, Jetty), stay on that path — see [VM and bare-metal deployment](/v4-0-1-snapshot/deployment/vm-deployment/).
- **Not Compose-only.** Single-host Docker Compose is simpler via [Docker Deployment](/v4-0-1-snapshot/deployment/docker-deployment/). `wheels deploy` adds value at 2+ servers.
...
<CardGrid>
  <LinkCard title="Your first deploy" href="/v4-0-1-snapshot/deployment/first-deploy/" description="Hands-on walkthrough — scaffold, configure, ship." />
  <LinkCard title="Migrating from Kamal" href="/v4-0-1-snapshot/deployment/migrating-from-kamal/" description="Full compatibility contract and switch-over checklist." />
  <LinkCard title="deploy.yml reference" href="/v4-0-1-snapshot/deployment/config-reference/" description="Every config key, what it does, defaults." />
  <LinkCard title="Deployment landing" href="/v4-0-1-snapshot/deployment/" description="When to reach for wheels deploy vs. other paths." />
</CardGrid>
