...
```bash title="wheels deploy secrets fetch (illustrative — requires vault credentials)"
...
```bash title="wheels deploy secrets extract (illustrative — uses pipe)"
echo "A=1
...
```bash title="wheels deploy secrets print (illustrative — requires resolved .kamal/secrets)"
wheels deploy secrets print
```
