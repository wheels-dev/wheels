...
- **Your deploy target is Kubernetes.** `wheels deploy` talks to `docker` remotely, not to a Kubernetes API. Use your normal k8s pipeline; the [Docker Deployment](/v4-0-1-snapshot/deployment/docker-deployment/) page covers image hygiene for that world.
- **You're deploying locally with Docker Compose only.** For single-host, developer-centric Compose setups, [Docker Deployment](/v4-0-1-snapshot/deployment/docker-deployment/) covers the Compose path end-to-end. `wheels deploy` adds value once you have two or more servers to coordinate.
- **Your target is a VM without containers.** If you run CommandBox or a standalone servlet container under systemd, stay on that path — [VM and Bare-metal Deployment](/v4-0-1-snapshot/deployment/vm-deployment/) covers it. `wheels deploy` is Docker-only on the server side; it does not grow a systemd mode.
...
If none of those apply, start with [Your First Deploy](/v4-0-1-snapshot/deployment/first-deploy/).
...
- `wheels deploy` — orchestration, dry-run, rollback, accessories, secrets adapters, hooks. See [Your First Deploy](/v4-0-1-snapshot/deployment/first-deploy/).
...
- The Linux hosts. Bare VMs, cloud instances, it doesn't matter — Docker is the only hard requirement. `wheels deploy bootstrap` installs Docker on a fresh host.
...
- Logs, metrics, and traces. See [Observability & Logging](/v4-0-1-snapshot/deployment/observability-and-logging/).
