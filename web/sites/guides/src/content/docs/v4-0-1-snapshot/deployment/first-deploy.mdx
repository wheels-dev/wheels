...
5. **Bootstrap Docker on the host (first time only).**
   ...
   ```bash {test:cli cmd="wheels deploy bootstrap"}
   ...
   :::note[`bootstrap` is the canonical form]
   The Kamal-style nested form `wheels deploy server bootstrap` is also accepted by the deploy dispatcher, but the wheels CLI's underlying runtime registers `server` as a top-level command (for managing the local Lucee dev server). When you type the three-token form at the shell, the CLI runtime grabs `server` before it reaches the deploy dispatcher. The flat `wheels deploy bootstrap` (and `wheels deploy exec`) aliases sidestep that collision. See [issue #2677](https://github.com/wheels-dev/wheels/issues/2677).
   :::
