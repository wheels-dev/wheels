...
| `kamal secrets fetch --from op://...` | `wheels deploy fetch-secrets --adapter=op --from=op://...` (flat alias — picocli intercepts the nested `secrets` form; see [#2697](https://github.com/wheels-dev/wheels/issues/2697)) |
...
- **`--version` and `--release` are interchangeable.** The brew and scoop wrappers transparently rewrite `--version[=val]` to `--release[=val]` when `deploy` is the first positional argument — the underlying LuCLI runtime reserves `--version` as a root-level boolean flag (issue [#2674](https://github.com/wheels-dev/wheels/issues/2674)), so the rewrite keeps the documented Kamal-compatible form working. If you ever invoke `wheels deploy` from a wrapper that pre-dates this rewrite (or directly against the LuCLI binary), pass `--release=<tag>` instead and the parser will treat it identically.
