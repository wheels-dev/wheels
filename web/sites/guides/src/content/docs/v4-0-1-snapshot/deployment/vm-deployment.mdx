...
<Aside type="note">
  You should already know how Wheels reads its configuration — see [Environments and Configuration](/v4-0-1-snapshot/core-concepts/environments-and-configuration/). You need root (or sudo) on a Linux host and a domain pointed at its public IP.
</Aside>
...
Wheels reads these at boot via `application.wo.env("VAR_NAME")`. See [Environments and Configuration](/v4-0-1-snapshot/core-concepts/environments-and-configuration/#secrets-handling) for where that lookup happens.
...
- `EnvironmentFile=` loads the `.env` into the service's environment. `WHEELS_ENV=production` is what flips Wheels into [production mode](/v4-0-1-snapshot/core-concepts/environments-and-configuration/#environment-detection).
...
<Aside type="note">
  `X-Forwarded-Proto` is what tells Wheels the request came in over HTTPS even though Lucee itself is listening on plain HTTP inside the box. Without it, `isSecure()`-style checks and protocol-aware redirects misfire. See [HTTPS Detection](/v4-0-1-snapshot/digging-deeper/security/https-detection/).
</Aside>
...
<CardGrid>
  ...
  <LinkCard
    ...
    href="/v4-0-1-snapshot/deployment/"
