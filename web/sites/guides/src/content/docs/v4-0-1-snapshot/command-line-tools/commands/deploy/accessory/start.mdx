...
`docker start <service>-<name>` on the accessory's pinned host(s). Fails if the container doesn't exist — use [`accessory boot`](/v4-0-1-snapshot/command-line-tools/commands/deploy/accessory/boot/) for first-time install.
