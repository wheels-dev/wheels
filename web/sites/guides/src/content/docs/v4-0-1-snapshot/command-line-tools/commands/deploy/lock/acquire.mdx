...
Emits the atomic lock-acquire command (`ln -s` against `/tmp/kamal_deploy_lock_<service>`) on any single host. Fails if the lock is already held — run [`lock status`](/v4-0-1-snapshot/command-line-tools/commands/deploy/lock/status/) to see who holds it.
