...
Per host: `docker stop kamal-proxy && docker rm kamal-proxy`, then the same `docker run` as [`proxy boot`](/v4-0-1-snapshot/command-line-tools/commands/deploy/proxy/boot/).
