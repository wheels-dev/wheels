...
<Aside type="note">
  You should already know your way around Git and GitHub, and have a local Wheels dev environment working. If `wheels start` doesn't run yet, start with [Running Tests Locally](/v4-0-1-snapshot/testing/running-tests-locally/) — it walks through `JAVA_HOME`, the `wheels` CLI, and SQLite.
</Aside>
...
<CardGrid>
  <LinkCard title="Running Tests Locally" href="/v4-0-1-snapshot/testing/running-tests-locally/" description="The test-local.sh script, filter flags, and the Docker cross-engine matrix." />
  <LinkCard title="Contributing overview" href="/v4-0-1-snapshot/contributing/" description="How the contributing section is organised and what else lives here." />
</CardGrid>
