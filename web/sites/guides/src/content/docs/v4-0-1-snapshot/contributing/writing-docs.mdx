...
- How to wire your page into `src/sidebars/v4-0-1-snapshot.json`
- The local workflow: write → `pnpm verify:docs` → `pnpm build` → PR

<Aside type="note">
  You should already be comfortable with Markdown and MDX. If you haven't opened a Wheels PR before, start with [Pull Requests](/v4-0-1-snapshot/contributing/pull-requests/). Security issues follow a different path — see the disclosure note on the [Contributing index](/v4-0-1-snapshot/contributing/).
</Aside>
...
- web/sites/guides/
  - src/
    - content/docs/
      - v4-0-0/           (the v4.0.0 stable docs)
      - v4-0-1-snapshot/  (you are here — the active snapshot)
        - start-here/
        - core-concepts/
        - basics/
        - digging-deeper/
...
The path on disk is the URL. `contributing/writing-docs.mdx` renders at `/v4-0-1-snapshot/contributing/writing-docs/`. Directory landing pages are named `index.mdx`. Use kebab-case filenames; the slug segment comes directly from the filename.
...
The sidebar is an explicit JSON tree — no auto-discovery. Edit `web/sites/guides/src/sidebars/v4-0-1-snapshot.json` and add your page under the matching section's `items` array.

```json title="web/sites/guides/src/sidebars/v4-0-1-snapshot.json"
{
  "label": "Contributing & Project",
  "link": "/v4-0-1-snapshot/contributing/",
  "items": [
    { "label": "Writing documentation", "link": "/v4-0-1-snapshot/contributing/writing-docs/" }
  ]
}
```
...
3. **Run the harness against your page.**
   ...
   pnpm verify:docs src/content/docs/v4-0-1-snapshot/contributing/writing-docs.mdx
   ```
...
5. **Wire the sidebar.** Add your entry to `src/sidebars/v4-0-1-snapshot.json`. Rebuild to confirm it renders.
...
<CardGrid>
  <LinkCard title="Pull Requests" href="/v4-0-1-snapshot/contributing/pull-requests/" description="Fork-and-pull workflow, branch naming, definition of done, and the review loop." />
  <LinkCard title="Coding Standards" href="/v4-0-1-snapshot/contributing/coding-standards/" description="Formatting rules, commit-message scopes, and the cross-engine gotchas to avoid." />
  <LinkCard title="Contributing & Project" href="/v4-0-1-snapshot/contributing/" description="How the project is run, who the core team is, and the four ways you can help." />
  <LinkCard title="Running Tests Locally" href="/v4-0-1-snapshot/testing/running-tests-locally/" description="A howto built in this style — a good reference when you're shaping your own." />
</CardGrid>
