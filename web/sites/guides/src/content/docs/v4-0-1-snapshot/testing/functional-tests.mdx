...
<Aside type="note">
  You should already know the request lifecycle. See [The Request Lifecycle](/v4-0-1-snapshot/core-concepts/request-lifecycle/). For the testing runtime, see [Controller Tests](/v4-0-1-snapshot/testing/controller-tests/).
</Aside>
...
`withHeader` attaches the request header; the assertion reads the response header the middleware set. See [CORS](/v4-0-1-snapshot/digging-deeper/cors/) for configuration options and preflight semantics.
...
Reuse one client so the rate-limit key (by default, the client IP) stays stable across the loop. See [Rate Limiting](/v4-0-1-snapshot/digging-deeper/rate-limiting/) for strategies (`fixedWindow`, `slidingWindow`, `tokenBucket`) and storage options.
...
`assertNotFound()` is sugar for `assertStatus(404)`. See [Route Model Binding](/v4-0-1-snapshot/digging-deeper/route-model-binding/) for how binding resolves the key into `params.post`.
...
Functional tests cover the server side only — what HTML the server ships on the first request. They can't exercise JavaScript, Turbo Frame/Stream swaps, client-side validation, or form submissions wired to `fetch`. For any flow whose behaviour emerges in the browser after the initial HTML, write a browser test instead. For pure server-rendered flows — classic form POST, redirect, GET — functional tests cover the same ground an order of magnitude faster. See [Browser Tests](/v4-0-1-snapshot/testing/browser-tests/) for the Playwright-backed DSL.

## Related guides

<CardGrid>
  <LinkCard title="Integration Tests" href="/v4-0-1-snapshot/testing/integration-tests/" description="Multi-step workflows, cookie-jar session state, and cross-controller assertions on database state." />
  <LinkCard title="Controller Tests" href="/v4-0-1-snapshot/testing/controller-tests/" description="Single-action response shape, redirects, filters, flash messages, and JSON responses." />
  <LinkCard title="Browser Tests" href="/v4-0-1-snapshot/testing/browser-tests/" description="Real browser automation via Playwright for JavaScript, Turbo, and UI-driven flows." />
  <LinkCard title="Middleware Pipeline" href="/v4-0-1-snapshot/core-concepts/middleware-pipeline/" description="How middleware wraps every request — the layer your functional tests exercise before the controller." />
</CardGrid>
