...
<Aside type="note">
  You should already know BDD basics. See [Model Tests](/v4-0-1-snapshot/testing/model-tests/). For non-browser HTTP tests that don't need JavaScript or Turbo, prefer [Controller Tests](/v4-0-1-snapshot/testing/controller-tests/), [Functional Tests](/v4-0-1-snapshot/testing/functional-tests/), or [View & Form Tests](/v4-0-1-snapshot/testing/view-and-form-tests/).
</Aside>
...
See [View & Form Tests](/v4-0-1-snapshot/testing/view-and-form-tests/) for the same `data-auto-id` pattern applied to non-browser tests, and [Forms and Form Helpers](/v4-0-1-snapshot/basics/forms-and-form-helpers/) for how the attribute is generated.
...
Set `WHEELS_BROWSER_TEST_BASE_URL`, set `this.baseUrl` in `beforeAll`, or rely on CGI auto-detect so `visit("/path")` resolves to the right origin. The env var is the standard CI escape hatch; `this.baseUrl` takes priority and lets individual specs target a different port without touching the environment. For local work against a non-8080 server (Titan on 60050, `wheels new` scaffolds on 60080), CGI auto-detect picks up the correct host and port automatically. See [CI Integration](/v4-0-1-snapshot/testing/ci-integration/) for the full workflow.
...
- Server-rendered HTML, redirects, status codes → [Controller Tests](/v4-0-1-snapshot/testing/controller-tests/)
- Rendered markup without JS → [View & Form Tests](/v4-0-1-snapshot/testing/view-and-form-tests/)
- Multi-controller flows without UI assertions → [Functional Tests](/v4-0-1-snapshot/testing/functional-tests/)
- Model behavior → [Model Tests](/v4-0-1-snapshot/testing/model-tests/)
...
<CardGrid>
  ...
  <LinkCard
    ...
    href="/v4-0-1-snapshot/basics/forms-and-form-helpers/"
