...
All six `accessControlAllow*` settings are initialized in the 4.0 security defaults and read on every request. The only thing that changed is the middleware's default — not the global setting.
