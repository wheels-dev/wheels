...
The middleware default drops `X-Auth-Token`, `X-Requested-By`, and `Origin` from the header allow-list. A like-for-like swap silently shrinks the list: preflight `OPTIONS` requests from clients that send those headers return no `Access-Control-Allow-Headers`, and the browser blocks the real request with no entry in your server logs.
