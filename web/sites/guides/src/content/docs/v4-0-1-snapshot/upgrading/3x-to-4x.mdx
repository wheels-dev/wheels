...
4. Install into `vendor/`, then reload. Wheels 4.1 will ship `wheels packages install`; until then, clone (or symlink a local checkout) into `vendor/`:

   ```bash title="your shell"
   gh repo clone yourorg/wheels-auditlog vendor/wheels-auditlog
   wheels reload
   ```
...
When you can't fix all eleven breaking items in one sprint, install the adapter, ship the upgrade, and modernize incrementally. Wheels 4.1 will support `wheels packages install wheels-legacy-adapter`; until then, the interim install is:

```bash title="your shell"
gh repo clone wheels-dev/wheels-legacy-adapter vendor/wheels-legacy-adapter
wheels reload
```
