...
The same issue affects `config/environment.cfm` when it reads the running environment from `.env` via `application.env`:

```cfm {test:compile} title="config/environment.cfm — unreliable pattern"
