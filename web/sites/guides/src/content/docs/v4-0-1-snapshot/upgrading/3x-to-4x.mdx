...
The reload password is read from `application.wheels.reloadPassword`, which is populated only by `set(reloadPassword = ...)` in `config/settings.cfm`. A value in `.env` alone does not reach `application.wheels.*` — you must wire it through explicitly:
...
- **"reloadPassword is empty" boot warning despite having `reloadPassword` in `.env`.** The fail-closed check reads `application.wheels.reloadPassword`, which is only populated by `set(reloadPassword = ...)` in `config/settings.cfm`. A `.env` value does not flow through automatically. Add `set(reloadPassword = env("WHEELS_RELOAD_PASSWORD", ""));` to `config/settings.cfm`. See #2631.
