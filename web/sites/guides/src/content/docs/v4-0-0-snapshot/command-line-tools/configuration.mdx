...
```json title="lucee.json — minimal example"
{
  "name": "my-wheels-app",
  "lucee": {
    "version": "7.0.1.100"
...
```jsonc title="lucee.json — all recognised keys"
{
  "name": "my-wheels-app",
  "lucee": {
    "version": "6.2.2.91",
...
```json title="lucee.json — environment profiles"
{
  "name": "my-app",
...
```bash title="selecting an environment profile"
...
```bash title="global flags before the subcommand"
...
```bash title="sharing home directories between wheels and lucli"
...
```bash title="juggling multiple Wheels installs on one machine"
# Project A
