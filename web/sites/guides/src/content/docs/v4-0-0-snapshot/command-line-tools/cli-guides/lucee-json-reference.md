...
For a full migration guide, see [Migrating from CommandBox](/v4-0-0-snapshot/command-line-tools/cli-guides/migration-from-commandbox/).
...
- Use **`.env` files** for values that change per environment (database credentials, ports, feature flags). See [Configuration Management](/v4-0-0-snapshot/command-line-tools/configuration/).
