...
```bash
wheels validate && wheels analyze
```

Run `validate` first: if it exits green (no errors, warnings optional), you haven't broken anything structural. Then run `analyze` for a broader look at health scores and anti-patterns. Chain them with `&&` so a failing `validate` short-circuits before you spend time reading a full analysis report.
