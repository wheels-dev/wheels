...
Websites often display the same thing on multiple pages. It could be an advertisement area that should be displayed in an entire section of a website or a shopping cart that is displayed while browsing products in a shop. You get the idea. To avoid duplicating code, you can place it in a file (the "partial" in Wheels terms) and include that file using [includePartial()](https://wheels.dev/api/v3.1.0/controller.includepartial.html) on the pages that need it.
...
This will wrap the partial with the code found in `/app/views/boxes/_blue.cfm`. Just like with other layouts, you use [includeContent()](https://wheels.dev/api/v3.1.0/controller.includecontent.html) to represent the partial's content.
...
Similar to passing in an object, you can also pass in a query result set to [includePartial()](https://wheels.dev/api/v3.1.0/controller.includepartial.html). Here's how that looks:
...
In this case, it's useful to use a partial to display each comment (using [includePartial()](https://wheels.dev/api/v3.1.0/controller.includepartial.html) as outlined above) and use the same partial when rendering the result of the AJAX request.
