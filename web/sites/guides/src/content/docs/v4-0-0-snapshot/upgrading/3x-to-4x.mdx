...
The `SecurityHeaders` middleware (#2036) emits `Strict-Transport-Security` by default in the `production` environment. If you terminate TLS outside the app and do not want HSTS emitted from here, disable it explicitly.

```cfm {test:compile} title="config/settings.cfm"
set(middleware = [ new wheels.middleware.SecurityHeaders(hsts=false) ]);
```
...
<CardGrid>
  <LinkCard
    title="Packages"
    description="The activation model, manifest fields, and error isolation for the packages/ → vendor/ system that replaces plugins/."
