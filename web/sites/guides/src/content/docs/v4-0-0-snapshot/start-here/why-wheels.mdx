...
<Aside type="note">
  This page assumes you've built a web app in at least one mature framework. If you haven't, skip to [Your First 15 Minutes](/v4-0-0-snapshot/start-here/first-15-minutes/) for a hands-on intro instead.
</Aside>
...
- **No bundler / Gemfile.** The framework lives in `vendor/wheels/`, copied into the app by the CLI. There's a package system (first-party packages activate via copy from `packages/` into `vendor/`), but there's no central package registry or lockfile — intentional.
...
| Package manager | Composer | No central registry; `packages/` → `vendor/` activation |
...
Livewire has no direct Wheels analog. For similar "server-rendered, JavaScript-light interactivity," use [Turbo Frames](/v4-0-0-snapshot/digging-deeper/) from the `hotwire` package.
...
| Migrations | `makemigrations` + `migrate` | `wheels generate migration` + `wheels dbmigrate latest` |
...
- [Welcome to Wheels](/v4-0-0-snapshot/start-here/welcome/) — orientation and structure
- [Your First 15 Minutes](/v4-0-0-snapshot/start-here/first-15-minutes/) — see Wheels in action
- [Installing Wheels](/v4-0-0-snapshot/start-here/installing/) — get the CLI
