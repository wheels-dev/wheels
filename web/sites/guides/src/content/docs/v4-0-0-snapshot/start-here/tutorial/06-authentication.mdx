...
- `beforeSave("hashPassword")` runs before every save. It only does anything when `this.password` is set — that is, when the caller just assigned a new password. It generates a per-user salt if one isn't already stored, hashes `password + salt` with SHA-256, stashes the result in `this.passwordHash`, and scrubs `this.password` so the plaintext never reaches the database.
...
3. Visit `/logout` (or submit a logout form) and then try `/posts/new` — you're redirected to `/login`.
...
The DI container you used to wire `SessionStrategy` in 6b is a core framework feature. See [The Dependency Injection Container](/v4-0-0-snapshot/core-concepts/dependency-injection/) for when to reach for it vs plain `new SomeClass()`.
