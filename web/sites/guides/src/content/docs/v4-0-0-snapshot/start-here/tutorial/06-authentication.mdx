...
**curl bracket-syntax and `@`**: a curl POST body using bracket-nested keys without an `=` separator — for example `--data-urlencode "user[email][alice@example.com]"` — is read by Lucee's form parser as a deeper nested-struct path. The email arrives in your controller as `params.user.email = {"alice@example.com": ""}` (a struct, not a string), and the next assignment via `setProperties()` is rejected with a `Wheels.PropertyIsIncorrectType` error from the model layer. The fix is to keep the `=` separator AND %-encode the brackets and the `@` in the body. The reliable shape is:
