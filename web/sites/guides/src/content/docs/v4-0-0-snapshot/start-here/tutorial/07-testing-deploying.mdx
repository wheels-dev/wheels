...
The `--filter` flag narrows the run to specs whose path contains the argument. Other filters accept `models`, `controllers`, or a full directory path. The exact syntax is still settling; see [CLI Reference](/v4-0-0-snapshot/command-line-tools/) for the current options.
...
<CardGrid>
  <LinkCard title="Bonus: Style with wheels-basecoat" href="/v4-0-0-snapshot/start-here/tutorial/08-bonus-basecoat/" description="Optional 30-min chapter. Install your first package and rewrite the post view with shadcn/ui-quality components." />
  ...
  <LinkCard title="CLI Reference" href="/v4-0-0-snapshot/command-line-tools/" description="Every wheels command." />
</CardGrid>
