...
<Aside type="note">
`wheels start` boots the Lucee Express bundled with the CLI — it does not start BoxLang. BoxLang developers manage their server through CommandBox (`box server start/stop/restart`). Generators (`wheels generate ...`) are pure file-system operations and work the same regardless of engine.
</Aside>
