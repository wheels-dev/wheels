...
For the bleeding-edge channel, re-run the bleeding-edge install snippet from the Linux tab above — it points at `wheels-dev/wheels-snapshots`, and `apt`/`dnf` will treat each new snapshot as an upgrade.
