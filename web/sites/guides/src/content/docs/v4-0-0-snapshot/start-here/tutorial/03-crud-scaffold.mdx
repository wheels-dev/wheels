...
- What packages are and how the `packages/` → `vendor/` activation model works
...
- `textField`, `textArea`, `select`, `dateTimeField` are object-bound helpers — `objectName="post"` plus `property="title"` becomes `name="post[title]"` with the current value pre-filled.
...
Core Wheels lives in `vendor/wheels/`. Optional features ship as separate packages. In the framework source tree those packages live at `packages/<name>/`. Activating one means copying (or symlinking) it into `vendor/<name>/`. On app boot, `PackageLoader.cfc` walks `vendor/*/package.json` and wires each package's mixins, services, and middleware into the framework.
...
Conceptually, activating Hotwire looks like this:
...
<Aside type="caution">
  On a fresh `wheels new` app, the `packages/` directory doesn't exist — it lives in the framework source tree. On this CLI version, you'd copy the package from the framework repo by hand. A future release will add a `wheels package install <name>` command. Part 4 wires Hotwire into the blog for real.
</Aside>
