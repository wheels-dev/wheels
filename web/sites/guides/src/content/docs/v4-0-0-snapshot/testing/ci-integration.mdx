...
- A minimum-viable GitHub Actions workflow for running `wheels test run` in CI
...
<Aside type="note">
  You should already know how to run tests locally. See [Running Tests Locally](/v4-0-0-snapshot/testing/running-tests-locally/) for `wheels test run`, `tools/test-local.sh`, and the Docker Compose map of ports and databases.
</Aside>
...
If you want the raw CLI instead of the bash wrapper, the equivalent is `wheels start` followed by `wheels test run --ci`.

## Reporter output for CI

The `wheels test run` command accepts a `--reporter=<name>` flag, but understanding what it does today avoids a surprise: the LuCLI implementation always requests JSON from the test endpoint and then prints a human-readable summary on success or failure details on failure. The flag is parsed but does not currently select between output formats — you get the same output regardless of `--reporter` value.
...
`pr.yml` covers Lucee 7 + SQLite. For the full cross-engine story, you run Docker Compose from the repo root and hit each engine's port directly — the same pattern documented in [Running Tests Locally](/v4-0-0-snapshot/testing/running-tests-locally/). Wrapped in a GitHub Actions matrix, every cell is an independent runner:
...
steps:
  - name: Run filtered suite
    run: wheels test run --filter=${{ matrix.directory }}
```
...
<CardGrid>
  <LinkCard title="Running Tests Locally" href="/v4-0-0-snapshot/testing/running-tests-locally/" description="The fast path with wheels test run and tools/test-local.sh, plus the Docker cross-engine matrix." />
  <LinkCard title="Browser Tests" href="/v4-0-0-snapshot/testing/browser-tests/" description="Installing Playwright, writing browser specs, and the beforeEach/afterEach lifecycle." />
