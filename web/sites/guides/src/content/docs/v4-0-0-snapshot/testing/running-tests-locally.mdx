...
- The fastest way to run the suite — `wheels test run` and `tools/test-local.sh`
...
## The fastest path — `wheels test run`

`wheels test run` is the single command you'll use most often. It detects a running Wheels server, reloads the app, hits the test runner, and prints pass/fail counts to your terminal. No Docker, no ceremony.
...
wheels test run model
wheels test run --filter=controller

# Pick a reporter — simple (default), verbose, or machine-readable
wheels test run --reporter=verbose
...
When you need to iterate on a single spec file without rebooting anything — fastest inner loop — hit the test runner directly over HTTP. This is what `wheels test run` and `tools/test-local.sh` call under the hood.
...
| Method | How |
|--------|-----|
| By directory (CLI) | `wheels test run model` or `wheels test run --filter=controller` |
...
Supported `db` values on the URL: `sqlite` (default for the script and local runs), `h2` (default on some engines), `mysql`, `postgresql`, `mssql`, `cockroachdb`. CockroachDB is currently soft-failed in CI — failures log warnings but don't block the build.
