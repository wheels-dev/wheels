...
If you're using [multi-tenancy](/v4-0-0-snapshot/working-with-wheels/multi-tenancy/), background jobs automatically capture the current tenant context when enqueued and restore it before `perform()` runs. No extra code is needed.
