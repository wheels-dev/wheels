...
Let's say that we wanted Wheels's built-in function [timeAgoInWords()](https://wheels.dev/api/v3.1.0/controller.timeAgoInWords.html) to return the time followed by the string " (approximately)":
...
See the chapter on [Configuration and Defaults](https://wheels.dev/3.1.0/guides/working-with-wheels/configuration-and-defaults) for more details about changing Wheels settings.
