...
<Aside type="note">
  You should already know how Wheels reads its configuration — see [Environments and Configuration](/v4-0-0-snapshot/core-concepts/environments-and-configuration/). You need root (or sudo) on a Linux host and a domain pointed at its public IP.
</Aside>
...
<Aside type="note">
  `X-Forwarded-Proto` is what tells Wheels the request came in over HTTPS even though Lucee itself is listening on plain HTTP inside the box. Without it, `isSecure()`-style checks and protocol-aware redirects misfire. See [HTTPS Detection](/v4-0-0-snapshot/digging-deeper/security/https-detection/).
</Aside>
