...
* You create a new `order` object using the [new()](https://wheels.dev/api/v3.1.0/model.new.html) method based on the incoming form parameters.
* You call the [save()](https://wheels.dev/api/v3.1.0/model.save.html) method on the order object, which will cause Wheels to first validate the object and then store it in the database if it passes validation.
* The next day, you call the [update()](https://wheels.dev/api/v3.1.0/model.update.html) method on the object because the user decided to change the shipping method for the order.
* Another day passes, and you call the [delete()](https://wheels.dev/api/v3.1.0/model.delete.html) method on the object because the visitor called in to cancel the order.
...
* [afterNew()](https://wheels.dev/api/v3.1.0/model.afternew.html) or [afterFind()](https://wheels.dev/api/v3.1.0/model.afterfind.html)
* [afterInitialization()](https://wheels.dev/api/v3.1.0/model.afterinitialization.html)
* [beforeValidation()](https://wheels.dev/api/v3.1.0/model.beforevalidation.html)
* [beforeValidationOnCreate()](https://wheels.dev/api/v3.1.0/model.beforevalidationoncreate.html) or [beforeValidationOnUpdate()](https://wheels.dev/api/v3.1.0/model.beforevalidationonupdate.html)
* [afterValidation()](https://wheels.dev/api/v3.1.0/model.aftervalidation.html)
* [afterValidationOnCreate()](https://wheels.dev/api/v3.1.0/model.aftervalidationoncreate.html) or [afterValidationOnUpdate()](https://wheels.dev/api/v3.1.0/model.aftervalidationonupdate.html)
* [beforeSave()](https://wheels.dev/api/v3.1.0/model.beforesave.html)
* [beforeCreate()](https://wheels.dev/api/v3.1.0/model.beforecreate.html) or [beforeUpdate()](https://wheels.dev/api/v3.1.0/model.beforeupdate.html)
* [afterCreate()](https://wheels.dev/api/v3.1.0/model.aftercreate.html) or [afterUpdate()](https://wheels.dev/api/v3.1.0/model.afterupdate.html)
* [afterSave()](https://wheels.dev/api/v3.1.0/model.aftersave.html)
* [beforeDelete()](https://wheels.dev/api/v3.1.0/model.beforedelete.html)
* [afterDelete()](https://wheels.dev/api/v3.1.0/model.afterdelete.html)

### Callback Life Cycle

As you can see above, there are a few places (5, to be exact) where one callback or the other will be executed, but not both.

The very first possible callback that can take place in an object's life cycle is either [afterNew()](https://wheels.dev/api/v3.1.0/model.afternew.html) or [afterFind](https://wheels.dev/api/v3.1.0/model.afterfind.html). The [afterNew()](https://wheels.dev/api/v3.1.0/model.afternew.html) callback methods are triggered when you create the object yourself for the very first time, for example, when using the [new()](https://wheels.dev/api/v3.1.0/model.new.html) method. [afterFind()](https://wheels.dev/api/v3.1.0/model.afterfind.html) is triggered when the object is created as a result of fetching a record from the database, for example, when using [findByKey()](https://wheels.dev/api/v3.1.0/model.findByKey.html). (There is some special behavior for this callback type that we'll explain in detail later on in this chapter).
...
If you want to completely break the callback chain for an object, you can do so by returning `false` from your callback method. (Otherwise, always return `true` or nothing at all.) As an example of breaking the callback chain, let's say you have called the [save()](https://wheels.dev/api/v3.1.0/model.save.html) method on a new object and the method you've registered with the [beforeCreate()](https://wheels.dev/api/v3.1.0/model.beforecreate.html) callback returns `false`. As a result, because the method you've registered with the [beforeCreate()](https://wheels.dev/api/v3.1.0/model.beforecreate.html) callback will exit the callback chain early by returning `false`, no record will be inserted in the database.
...
When you read about the [afterFind()](https://wheels.dev/api/v3.1.0/model.afterfind.html) callback above, you may have thought that it must surely only work for [findOne()](https://wheels.dev/api/v3.1.0/model.findone.html)/ [findByKey()](https://wheels.dev/api/v3.1.0/model.findbykey.html) calls but not for [findAll()](https://wheels.dev/api/v3.1.0/model.findall.html) because those calls return query result sets by default, not objects.

Believe it or not, but callbacks are even triggered on [findAll()](https://wheels.dev/api/v3.1.0/model.findall.html)! You do need to write your callback code differently though because there will be no `this` scope in the query object. Instead of modifying properties in the `this` scope like you normally would, the properties are passed to the callback method via the `arguments` struct.
...
Please note that if you use the [updateAll()](https://wheels.dev/api/v3.1.0/model.updateall.html) or the [deleteAll()](https://wheels.dev/api/v3.1.0/model.deleteall.html) methods in Wheels, they will not instantiate objects by default, and thus any callbacks will be skipped. This is good for performance reasons because if you update 1,000 records at once, you probably don't want to run the callbacks on each object. Especially not if they involve database calls.

However, if you want to execute all callbacks in those methods as well, all you have to do is pass in `instantiate=true`to the [updateAll()](https://wheels.dev/api/v3.1.0/model.updateall.html)/ [deleteAll()](https://wheels.dev/api/v3.1.0/model.deleteall.html) methods.
