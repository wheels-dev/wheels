...
_Nested properties_ in Wheels makes this scenario dead simple. With a configuration using the [nestedProperties()](https://wheels.dev/api/v3.1.0/model.nestedproperties.html)function in your model's `config()` method, you can save changes to that model and its associated models in a single call with [save()](https://wheels.dev/api/v3.1.0/model.save.html), [create()](https://wheels.dev/api/v3.1.0/model.create.html), or [update()](https://wheels.dev/api/v3.1.0/model.update.html).
...
By adding the call to [nestedProperties()](https://wheels.dev/api/v3.1.0/model.nestedproperties.html) in the model, you can create both the `user` object and the `profile` object in a single call to [create()](https://wheels.dev/api/v3.1.0/model.create.html).
...
In this example, we have added the `addresses` association to the call to [nestedProperties()](https://wheels.dev/api/v3.1.0/model.nestedproperties.html).
...
Even with a complex form with a number of child objects, Wheels will save all of the data through its parent's [save()](https://wheels.dev/api/v3.1.0/model.save.html), [update()](https://wheels.dev/api/v3.1.0/model.update.html), or [create()](https://wheels.dev/api/v3.1.0/model.create.html) methods.
...
See the chapter on [Transactions](https://wheels.dev/3.1.0/guides/database-interaction-through-models/transactions) for more details.
...
Consider the many-to-many associations related to `customers, publications`, and `subscriptions`, straight from the [Associations](https://wheels.dev/3.1.0/guides/database-interaction-through-models/associations) chapter.
...
When it's time to save `customer`s' subscriptions in the `subscriptions` join table, one approach is to loop through data submitted by [checkBoxTag()](https://wheels.dev/api/v3.1.0/controller.checkboxtag.html)s from your form, populate `subscription` model objects with the data, and call [save()](https://wheels.dev/api/v3.1.0/model.save.html). This approach is valid, but it is quite cumbersome. Fortunately, there is a simpler way.
...
The main point of interest in this example is the `<fieldset>` for Subscriptions, which loops through the query of `publications` and uses the [hasManyCheckBox()](https://wheels.dev/api/v3.1.0/controller.hasmanycheckbox.html) form helper. This helper is similar to [checkBox()](https://wheels.dev/api/v3.1.0/controller.checkbox.html) and [checkBoxTag()](https://wheels.dev/api/v3.1.0/controller.checkboxtag.html), but it is specifically designed for building form data related by associations. (Note that [checkBox()](https://wheels.dev/api/v3.1.0/controller.checkbox.html) is primarily designed for columns in your table that store a single `true/false` value, so that is the big difference.)

Notice that the `objectName` argument passed to [hasManyCheckBox()](https://wheels.dev/api/v3.1.0/controller.hasmanycheckbox.html) is the parent `customer` object and the `associations` argument contains the name of the related association. Wheels will build a form variable named in a way that the `customer` object is automatically bound to the `subscriptions` association.
...
* All of these database queries will be wrapped in a [Transaction](https://wheels.dev/3.1.0/guides/database-interaction-through-models/transactions) . If any of the above updates don't pass validation or if the database queries fail, the transaction will roll back.
