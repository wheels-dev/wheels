...
Well, good news. Of course you don't need to do this; just use the built-in functions [sum()](https://wheels.dev/api/v3.1.0/model.sum.html), [minimum()](https://wheels.dev/api/v3.1.0/model.minimum.html), [maximum()](https://wheels.dev/api/v3.1.0/model.maximum.html), [average()](https://wheels.dev/api/v3.1.0/model.average.html) and [count()](https://wheels.dev/api/v3.1.0/model.count.html).

Let's start with the [count()](https://wheels.dev/api/v3.1.0/model.count.html) function, shall we?
...
What if you only want to count authors with a last name starting with "A"? Like the [findAll()](https://wheels.dev/api/v3.1.0/model.findall.html) function, [count()](https://wheels.dev/api/v3.1.0/model.count.html) will accept a `where` argument, so you can do this:
...
Just like in the [findAll()](https://wheels.dev/api/v3.1.0/model.findall.html) function, you can now use the `include` argument to reference other tables.
...
OK, so now we've covered the [count()](https://wheels.dev/api/v3.1.0/model.count.html) function, but there are a few other functions you can use as well to get column statistics.

### Getting an Average

You can use the [average()](https://wheels.dev/api/v3.1.0/model.average.html) function to get the average value on any given column. The difference between this function and the [count()](https://wheels.dev/api/v3.1.0/model.count.html) function is that this operates on a single column, while the [count()](https://wheels.dev/api/v3.1.0/model.count.html) function operates on complete records. Therefore, you need to pass in the name of the property you want to get an average for.
...
To get the highest and lowest values for a property, you can use the [minimum()](https://wheels.dev/api/v3.1.0/model.minimum.html) and [maximum()](https://wheels.dev/api/v3.1.0/model.maximum.html) functions.
...
The last of the column statistics functions is the [sum()](https://wheels.dev/api/v3.1.0/model.sum.html) function.

As you have probably already figured out, [sum()](https://wheels.dev/api/v3.1.0/model.sum.html) adds all values for a given property and returns the result. You can use the same arguments as with the other functions (`property, where, include`, and `distinct`).
