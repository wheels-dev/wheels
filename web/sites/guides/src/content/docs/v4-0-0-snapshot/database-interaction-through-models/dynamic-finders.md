...
In the examples above, we've used the [findOne()](https://wheels.dev/api/v3.1.0/model.findone.html) method, but you can use the same approach on a [findAll()](https://wheels.dev/api/v3.1.0/model.findall.html) method as well.

### Passing in Other Finder Parameters

In the background, these dynamically-named methods just pass along execution to [findOne()](https://wheels.dev/api/v3.1.0/model.findone.html) or [findAll()](https://wheels.dev/api/v3.1.0/model.findall.html). This means that you can also pass in any arguments that are accepted by those two methods.
