...
<Aside type="note">
  You should already know how models work — see [Models and the ORM](/v4-0-0-snapshot/basics/models-and-the-orm/). Jobs persist to a `wheels_jobs` table; every enqueued job is a row. The table auto-creates on first use, so there's no migration to run.
</Aside>
