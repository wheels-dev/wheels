...
The bar respects the `enableDebug` application setting. By default, Wheels sets this to `true` in the `development` and `testing` environments and `false` in `production` and `maintenance`. You can override it explicitly in `config/environment.cfm`:

```cfm title="config/environment.cfm"
<cfset set(enableDebug=true)>
```

Setting `enableDebug=false` removes the bar entirely — no HTML is injected into responses.
...
| DB Adapter | The active database adapter (MySQL, PostgreSQL, SQL Server, etc.) |
...
|---|---|---|
| `enableDebug` | `true` in development/testing | Show or hide the debug bar entirely |
...
| `enablePublicComponent` | `true` | Show the Tools tab and its link grid |
