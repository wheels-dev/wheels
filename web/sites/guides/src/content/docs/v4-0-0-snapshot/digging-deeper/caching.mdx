...
Pass `static=true` and Wheels uses Lucee's `cfcache` server-side cache, which short-circuits the request even earlier — before filters don't run at all. Use this for marketing pages, pricing tables, anything truly public:
...
For multi-tenant apps, see [Multi-tenancy](/v4-0-0-snapshot/digging-deeper/multi-tenancy/) for the tenant-scoped key pattern.
...
Configure edge caching at the deploy layer, not in Wheels. The [Deployment](/v4-0-0-snapshot/deployment/) guide covers setting `Cache-Control` headers from your controller, letting Kamal's proxy or your CDN honor them, and purging the edge cache when a model saves.
