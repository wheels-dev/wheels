...
<Aside type="note">
  You should already know: Wheels controllers, the `params` struct, and form helpers. Finish the [tutorial](/v4-0-0-snapshot/start-here/tutorial/) first. Skim [Forms and Form Helpers](/v4-0-0-snapshot/basics/forms-and-form-helpers/) and [Controllers and Actions](/v4-0-0-snapshot/basics/controllers-and-actions/) if either feels shaky.
</Aside>
...
- **Rate limit.** Both sides. Upload endpoints invite abuse (disk fill, compute-heavy validation); download endpoints invite scraping. Pair with [Rate Limiting](/v4-0-0-snapshot/digging-deeper/rate-limiting/) per IP and per user.

## Related guides

<CardGrid>
  <LinkCard title="Forms and Form Helpers" href="/v4-0-0-snapshot/basics/forms-and-form-helpers/" description="Where fileField and startFormTag come from." />
  <LinkCard title="Controllers and Actions" href="/v4-0-0-snapshot/basics/controllers-and-actions/" description="Filters, params, and the action lifecycle the upload flow sits inside." />
  <LinkCard title="Background Jobs" href="/v4-0-0-snapshot/digging-deeper/background-jobs/" description="Generate thumbnails, transcode video, or virus-scan uploads on a worker." />
  <LinkCard title="Rate Limiting" href="/v4-0-0-snapshot/digging-deeper/rate-limiting/" description="Throttle both upload and download endpoints per IP and per user." />
</CardGrid>
