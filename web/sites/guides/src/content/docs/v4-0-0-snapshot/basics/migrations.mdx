...
<Aside type="note">
  A migration describes schema — the tables and columns your models map to. For the model side — finders, validations, callbacks — see [Models and the ORM](/v4-0-0-snapshot/basics/models-and-the-orm/).
</Aside>

## Running migrations

Four subcommands cover the day-to-day workflow. Each one connects to your app's datasource and reads the migration history out of `schema_migrations`, so the app has to be running (or at least reachable) for them to work — see [Installing Wheels](/v4-0-0-snapshot/start-here/installing/) for the server setup.
...
2. `t.timestamps()` creates `createdAt`, `updatedAt`, **and** a soft-delete column (`deletedAt`). Don't also add those columns by hand — you'll get duplicate-column errors. See [Models and the ORM](/v4-0-0-snapshot/basics/models-and-the-orm/) for how Wheels populates them automatically on save.
...
Pass `unique=true` on `addIndex` to create a unique constraint, which is what [`validatesUniquenessOf`](/v4-0-0-snapshot/basics/validation-and-errors/) should sit on top of to race-proof the check at the database level.
...
<CardGrid>
  <LinkCard title="Models and the ORM" href="/v4-0-0-snapshot/basics/models-and-the-orm/" description="Models map to the tables migrations create — finders, persistence, validations." />
  <LinkCard title="Associations" href="/v4-0-0-snapshot/basics/associations/" description="The foreign-key columns that hasMany and belongsTo read from — defined here, declared there." />
  <LinkCard title="Seeding" href="/v4-0-0-snapshot/basics/seeding/" description="Idempotent seed data via seedOnce() — the right home for reference records." />
  <LinkCard title="Database and Multiple Datasources" href="/v4-0-0-snapshot/basics/database-and-multiple-datasources/" description="Configuring the connection migrations run against, and targeting a non-default datasource." />
</CardGrid>
