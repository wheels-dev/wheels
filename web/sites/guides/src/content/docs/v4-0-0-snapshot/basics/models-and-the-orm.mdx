...
<Aside type="note">
  You should already know why models carry behavior. See [ORM Philosophy](/v4-0-0-snapshot/core-concepts/orm-philosophy/).
</Aside>
...
- `dataSource("legacy_db")` — non-default datasource. See [Database and Multiple Datasources](/v4-0-0-snapshot/basics/database-and-multiple-datasources/) when this lands for the wider multi-database story.
...
Both `save()` and `create()` run validations first; if any fail, the write is skipped and the record is returned with errors attached. See [Validation and Error Display](/v4-0-0-snapshot/basics/validation-and-errors/) for how that flow surfaces to the view.
...
<CardGrid>
  <LinkCard title="Associations" href="/v4-0-0-snapshot/basics/associations/" description="hasMany, belongsTo, hasOne — navigating relationships with include and dependent." />
  <LinkCard title="Query Builder and Scopes" href="/v4-0-0-snapshot/basics/query-builder-and-scopes/" description="Injection-safe query composition for anything more complex than a where string." />
  <LinkCard title="Validation and Error Display" href="/v4-0-0-snapshot/basics/validation-and-errors/" description="How save returns false, how the errors flow back to the view." />
  <LinkCard title="Migrations" href="/v4-0-0-snapshot/basics/migrations/" description="Defining the tables those models map to." />
</CardGrid>
