...
<Aside type="note">
  You should already know how the router matches URLs from [How Routing Works](/v4-0-0-snapshot/core-concepts/how-routing-works/). This page is the hands-on version — every example is a real `config/routes.cfm` fragment.
</Aside>
...
Rename the resource once in `routes.cfm` and every helper updates. See [Forms and Form Helpers](/v4-0-0-snapshot/basics/forms-and-form-helpers/) for `startFormTag` examples in context.
...
<CardGrid>
  <LinkCard title="How Routing Works" href="/v4-0-0-snapshot/core-concepts/how-routing-works/" description="The match algorithm, resource expansion, and route model binding." />
  <LinkCard title="Controllers and Actions" href="/v4-0-0-snapshot/basics/controllers-and-actions/" description="Where route params land and what to do with them." />
  <LinkCard title="Middleware Pipeline" href="/v4-0-0-snapshot/core-concepts/middleware-pipeline/" description="What runs before the router sees the request." />
</CardGrid>
