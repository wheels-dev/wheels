...
<Aside type="note">
  You should already know your way around Git and GitHub, and have a local Wheels dev environment working. If `wheels start` doesn't run yet, start with [Running Tests Locally](/v4-0-0/testing/running-tests-locally/) — it walks through `JAVA_HOME`, the `wheels` CLI, and SQLite.
</Aside>
...
- **Cross-engine fixes** — if CI flags Adobe CF or BoxLang failures a local Lucee run didn't catch, consult the cross-engine gotcha list in [Running Tests Locally](/v4-0-0/testing/running-tests-locally/#cross-engine-gotchas). Verify your fix against the local Docker container for that engine before pushing.
...
<CardGrid>
  <LinkCard title="Running Tests Locally" href="/v4-0-0/testing/running-tests-locally/" description="The test-local.sh script, filter flags, and the Docker cross-engine matrix." />
  <LinkCard title="Contributing overview" href="/v4-0-0/contributing/" description="How the contributing section is organised and what else lives here." />
</CardGrid>
