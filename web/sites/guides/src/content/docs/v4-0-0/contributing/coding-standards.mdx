...
<Aside type="note">
  You should already be comfortable with CFML components and Wheels' model/controller basics. If you're about to edit `vendor/wheels/`, read the [Cross-engine rules](#cross-engine-rules) below before anything else.
</Aside>
...
- **Use `CURRENT_TIMESTAMP` in migration SQL** — it is ANSI SQL and works on MySQL, PostgreSQL, SQL Server, H2, and SQLite. `NOW()` is not rewritten by any adapter and fails on SQLite (the `wheels new` default) and SQL Server.
...
```bash title="your shell — from the wheels repo root" {test:cli cmd="echo 'illustrative'"}
tools/test-matrix.sh lucee6,adobe2025 sqlite
```

The script wraps the repo-root `compose.yml` (the same one CI's compat matrix uses), waits for the engines to come up, and runs the suite against each. To iterate manually instead, `docker compose up -d lucee6 adobe2025` from the repo root and curl the runners yourself:
...
If either returns HTTP 417 (test failures) or non-zero counts, fix before pushing. See [Running tests locally](/v4-0-0/testing/running-tests-locally/) for the full engine-port table and database targets.
...
- **New public API** (framework function, CLI command, middleware, package hook) — add a guide page under the matching section of `web/sites/guides/src/content/docs/v4-0-0/`. Pick the Diátaxis type (`tutorial`, `howto`, `concept`, `reference`) and follow `web/sites/guides/STYLE.md`.
...
<CardGrid>
  <LinkCard title="Running tests locally" href="/v4-0-0/testing/running-tests-locally/" description="The wheels test CLI, tools/test-local.sh, and the Docker cross-engine matrix." />
  <LinkCard title="Conventions over configuration" href="/v4-0-0/core-concepts/conventions-over-configuration/" description="Why Wheels names things the way it does, and when to override." />
  <LinkCard title="Submitting pull requests" href="/v4-0-0/contributing/pull-requests/" description="The fork-branch-test-submit flow and how to pass commitlint on the first try." />
</CardGrid>
