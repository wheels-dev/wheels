...
Break the convention and you configure your way back. Rename a table, and the model needs `table("tbl_posts")`. Rename the primary key, and it needs `setPrimaryKey("post_id")`. The defaults exist so the common case needs no configuration — the overrides exist for the rest.
