...
The static-first step means declaration order does **not** decide static-vs-placeholder conflicts — only placeholder-vs-placeholder ones and ties between identical static patterns. These are the intended semantics, settled in [#3073](https://github.com/wheels-dev/wheels/issues/3073) and pinned by the framework test suite: a literal path always routes to its literal route, so the classic bug where an earlier `/posts/[key]` steals `/posts/featured` cannot happen in Wheels.
...
- [The Request Lifecycle](/v4-0-0/core-concepts/request-lifecycle/) — routing is the first stage of dispatch
- [Middleware Pipeline](/v4-0-0/core-concepts/middleware-pipeline/) — runs after the route is matched, before the controller
