...
The `wheels` CLI is the fastest way in and what these guides use by default. It creates apps, runs migrations, generates scaffolds, starts the dev server, runs tests, and ships your code. Under the hood it wraps a Java-based CFML runtime; you rarely think about that. It's not the only supported path, though: CommandBox teams can install and serve the framework from ForgeBox (see [Installing with CommandBox](/v4-0-0/start-here/installing-with-commandbox/)), and downloading a release directly from [GitHub](https://github.com/wheels-dev/wheels/releases) and wiring it into your own server setup works too.
...
- [Why Wheels?](/v4-0-0/start-here/why-wheels/) — head-to-head with Rails, Laravel, and Django
- [Core Concepts](/v4-0-0/core-concepts/) — the mental model
- [CLI Reference](/v4-0-0/command-line-tools/) — every `wheels` command
