...
2. Add the Wheels bucket and install a channel:
   ...
   Scoop pulls `java/openjdk21` automatically as a dependency. Both packages expose the same `wheels` shim on PATH — Scoop refuses to install both at once. See [Release Channels](/v4-0-0/start-here/release-channels/) for the comparison.
