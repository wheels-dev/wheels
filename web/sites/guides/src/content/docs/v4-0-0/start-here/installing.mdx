...
The apt and yum repos are GPG-signed with the Wheels Distribution key (fingerprint `6872 16C9 32B4 9F03 94E0 9AED 5D89 AF8F 9C9B 8CFB`). `apt`/`dnf` verify the signature on every update, so a network-level attacker can't substitute the package contents. The public key is published at `https://apt.wheels.dev/wheels.gpg` and `https://yum.wheels.dev/wheels.gpg` in ASCII-armored format. The snippets below convert it to binary with `gpg --dearmor` before writing to `/usr/share/keyrings/` — apt requires binary format and will reject an ASCII-armored file with an "unsupported filetype" warning otherwise. The sources.list / `.repo` snippets wire the signature check in automatically.
</Aside>
