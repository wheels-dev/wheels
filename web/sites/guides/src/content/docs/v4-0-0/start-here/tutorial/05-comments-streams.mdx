...
The `hasMany` / `belongsTo` pattern you just wired up — plus `dependent="delete"` and `include="comments"` — is covered in full at [Associations](/v4-0-0/basics/associations/). For dynamic queries beyond raw `where=` strings, see [Query Builder and Scopes](/v4-0-0/basics/query-builder-and-scopes/).
