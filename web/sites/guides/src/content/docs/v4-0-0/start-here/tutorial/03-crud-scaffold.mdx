...
In [Part 4: Validations + Turbo Frames](/v4-0-0/start-here/tutorial/04-validations-frames/) you'll add `validatesPresenceOf` to the `Post` model, wrap the form in a `<turbo-frame>`, and watch inline validation errors update without a page reload.

The scaffold's generated controller is a great starting point for [Controllers and Actions](/v4-0-0/basics/controllers-and-actions/). The `_form.cfm` it emits is unpacked in [Views, Layouts, Partials](/v4-0-0/basics/views-layouts-partials/).
