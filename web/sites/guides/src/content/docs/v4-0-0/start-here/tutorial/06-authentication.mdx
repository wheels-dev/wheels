...
On any reload — `wheels reload` or `wheels stop && wheels start` — this registers the strategy exactly once. (An authorized `wheels reload` calls `applicationStop()` so the next request re-fires `onApplicationStart` in full; a missing or wrong reload password silently skips the restart, so `wheels stop && wheels start` is the safer fallback if you're unsure.) The `hasStrategy` check keeps a second reload from stacking duplicates.
...
**"6b: `authenticator` service not found."** Three things to check, in this order. **(1) Does the file have a `<cfscript>` wrapper?** Without it, Lucee treats the body as markup — you'll see the bare `local.di = injector();` lines printed at the top of every page after a cold restart, and the registration code never runs. Compare against `config/settings.cfm` if unsure of the shape. **(2) Did you reload or restart after editing `config/services.cfm`?** An authorized `wheels reload` re-fires `onApplicationStart` in full (re-running `config/services.cfm`). A missing or wrong reload password silently skips the restart — if unsure, run `wheels stop && wheels start` instead. **(3) Component path typo?** The `.to(...)` argument must be the exact dotted path — `wheels.auth.Authenticator`, not `Authenticator` — and the `injector()` call has to come first.
