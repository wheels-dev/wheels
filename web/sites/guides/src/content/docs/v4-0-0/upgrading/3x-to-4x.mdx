...
See [Packages](/v4-0-0/digging-deeper/packages/) for manifest fields, per-method mixin overrides via `mixin="model"` or `mixin="none"` annotations, and error isolation. If you need a staged migration, the [`wheels-legacy-adapter`](https://github.com/wheels-dev/wheels-legacy-adapter) package ships deprecation logging and API shims for 3.x patterns.
