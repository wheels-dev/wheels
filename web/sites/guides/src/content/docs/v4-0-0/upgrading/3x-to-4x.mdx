...
- **"CSRF token invalid" on forms after upgrade.** Either the encryption key rotated (set `csrfCookieEncryptionSecretKey` explicitly when using `csrfStore="cookie"`) or a third-party embed is hitting `SameSite`. See #2035, #2054.
