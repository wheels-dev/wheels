...
component implements="wheels.middleware.MiddlewareInterface" output="false" {
    ...
    public string function handle(required struct request, required any next) {
        // Read the real `cgi` scope. The middleware request context only
        // carries {params, route, pathInfo, method} — it has no `cgi` key,
        // so `request.cgi.http_x_request_id` would always be empty.
        local.inbound = cgi.http_x_request_id ?: "";
