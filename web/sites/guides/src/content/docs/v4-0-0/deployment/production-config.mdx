...
- URL-based environment switching is disabled unless you explicitly re-enable it (`onapplicationstart.cfc:350-360`).
...
Wheels disables URL-based switching **automatically** in production, testing, and maintenance environments unless you explicitly `set(allowEnvironmentSwitchViaUrl=true)` in your per-env settings (`onapplicationstart.cfc:350-360`). Leave it disabled. A cached reload password plus URL switching plus production is a bad combination.
