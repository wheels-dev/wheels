...
<Aside type="note">
  You should already know MVC basics. See [MVC in Wheels](/v4-0-0-snapshot/core-concepts/mvc-in-wheels/).
</Aside>
...
`renderPartial` is the controller-side equivalent — use it from an action when you want to return a partial as the full response (Turbo Frame swaps, AJAX updates). Same partial file, same arguments; the difference is who calls it. See [Controllers and Actions](/v4-0-0-snapshot/basics/controllers-and-actions/).
...
Wheels 4.0 form helpers escape their input by default (see [Forms and Form Helpers](/v4-0-0-snapshot/basics/forms-and-form-helpers/)). Outside the form helpers, escaping is on you. Never interpolate user input into a view unescaped — that's the classic XSS gap.
...
Form and field helpers (`textField`, `emailField`, `submitTag`, and friends) get their own page — see [Forms and Form Helpers](/v4-0-0-snapshot/basics/forms-and-form-helpers/).

## Related guides

<CardGrid>
  <LinkCard title="Controllers and Actions" href="/v4-0-0-snapshot/basics/controllers-and-actions/" description="What runs before the view — filters, params, render overrides." />
  <LinkCard title="Forms and Form Helpers" href="/v4-0-0-snapshot/basics/forms-and-form-helpers/" description="Object-bound inputs, HTML5 field helpers, submit tags." />
  <LinkCard title="Conventions over Configuration" href="/v4-0-0-snapshot/core-concepts/conventions-over-configuration/" description="Why the filename conventions on this page are load-bearing." />
</CardGrid>
