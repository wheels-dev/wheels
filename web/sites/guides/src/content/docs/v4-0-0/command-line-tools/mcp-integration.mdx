...
| `upgrade` | Scan the project for breaking changes (`check`, read-only) or swap `vendor/wheels/` to a newer release (`apply`, writes with a backup). |
...
- `jobs` — `jobs work` is a long-lived worker poll loop with no single-call MCP semantics.
- `mcpToolSpecs` — the per-tool inputSchema registry LuCLI reads; not itself a tool.
...
**The server never responds / the IDE shows zero tools.** The most common cause: GUI-launched editors (VS Code, Cursor, Claude Desktop opened from the dock or Start menu) don't inherit your shell's `PATH`, so the bare `"command": "wheels"` in `.mcp.json` can't be found — and most clients fail silently. Fix it by using the absolute binary path:

```json title=".mcp.json"
{"mcpServers": {"wheels": {"command": "/opt/homebrew/bin/wheels", "args": ["mcp", "wheels"]}}}
```

(`which wheels` in a terminal prints the path to use — e.g. `/usr/bin/wheels` on apt/yum installs, `%USERPROFILE%\scoop\shims\wheels.exe` on Scoop.) Launching the editor from a terminal (`code .`) also works, since the child inherits the shell environment.

**Tools appear but calls fail.** Run `wheels mcp wheels` by hand in a terminal at your project root — startup errors that the IDE swallows print directly there.
...
<CardGrid>
  ...
  <LinkCard
    title="Installation"
    href="../installation/"
    description="Install wheels via Homebrew, Scoop, apt/yum, or a manual JAR download."
  />
  <LinkCard
    title="CLI Reference"
    href="../"
