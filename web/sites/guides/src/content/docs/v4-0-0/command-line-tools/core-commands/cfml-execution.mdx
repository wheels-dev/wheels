...
```bash title="synopsis — illustrative"
...
```bash {test:cli cmd="wheels cfml writeOutput(1+2)" asserts-stdout="3"}
...
```bash title="illustrative — multi-command example"
...
```text title="example REPL session — illustrative"
$ wheels repl
