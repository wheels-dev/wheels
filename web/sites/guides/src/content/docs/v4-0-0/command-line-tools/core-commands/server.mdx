...
- Tailing server, access, or error logs without hunting through `~/.wheels/servers/<name>/logs` (`~/.lucli/` on standalone LuCLI).
...
<CardGrid>
  ...
  <LinkCard
    ...
    description="Drive the CLI's MCP tools from an AI IDE over stdio. Server start/stop are deliberately hidden from MCP — use the terminal for lifecycle."
  />
</CardGrid>
