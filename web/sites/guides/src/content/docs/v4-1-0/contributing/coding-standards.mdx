...
If either returns HTTP 417 (test failures) or non-zero counts, fix before pushing. See [Running tests locally](/v4-1-0/testing/running-tests-locally/) for the full engine-port table and database targets.
...
- **New public API** (framework function, CLI command, middleware, package hook) — add a guide page under the matching section of `web/sites/guides/src/content/docs/v4-1-0/`. Pick the Diátaxis type (`tutorial`, `howto`, `concept`, `reference`) and follow `web/sites/guides/STYLE.md`.
...
<CardGrid>
  <LinkCard title="Running tests locally" href="/v4-1-0/testing/running-tests-locally/" description="The wheels test CLI, tools/test-local.sh, and the Docker cross-engine matrix." />
  <LinkCard title="Conventions over configuration" href="/v4-1-0/core-concepts/conventions-over-configuration/" description="Why Wheels names things the way it does, and when to override." />
  <LinkCard title="Submitting pull requests" href="/v4-1-0/contributing/pull-requests/" description="The fork-branch-test-submit flow and how to pass commitlint on the first try." />
</CardGrid>
