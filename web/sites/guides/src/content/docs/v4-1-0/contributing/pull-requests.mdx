...
<Aside type="note">
  You should already know your way around Git and GitHub, and have a local Wheels dev environment working. If `wheels start` doesn't run yet, start with [Running Tests Locally](/v4-1-0/testing/running-tests-locally/) — it walks through `JAVA_HOME`, the `wheels` CLI, and SQLite.
</Aside>
...
The script creates the SQLite test databases, starts a disposable LuCLI server on port 8080 if one isn't already up, runs the suite, prints a coloured pass/fail summary, and cleans up on exit. See [Running Tests Locally](/v4-1-0/testing/running-tests-locally/) for the full command surface, filter aliases, and the Docker cross-engine matrix for pre-merge verification.
...
- **Cross-engine fixes** — if CI flags Adobe CF or BoxLang failures a local Lucee run didn't catch, consult the cross-engine gotcha list in [Running Tests Locally](/v4-1-0/testing/running-tests-locally/#cross-engine-gotchas). Verify your fix against the local Docker container for that engine before pushing.
...
<CardGrid>
  <LinkCard title="Running Tests Locally" href="/v4-1-0/testing/running-tests-locally/" description="The test-local.sh script, filter flags, and the Docker cross-engine matrix." />
  <LinkCard title="Contributing overview" href="/v4-1-0/contributing/" description="How the contributing section is organised and what else lives here." />
</CardGrid>
