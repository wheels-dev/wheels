...
This page is for people hacking on **Wheels itself** — the code in `vendor/wheels/` of the [framework repo](https://github.com/wheels-dev/wheels). If you're testing *your application*, you want [Running Tests Locally](/v4-1-0/testing/running-tests-locally/); nothing on this page exists in a scaffolded app.
...
# Summarize — fail closed on #3083 honesty fields, not just Fail/Error
for f in /tmp/lucee7.json /tmp/adobe2025.json; do
  python3 -c "
import json, sys
d = json.load(open('$f'))
print('$f', d.get('totalPass',0), 'pass', d.get('totalFail',0), 'fail', d.get('totalError',0), 'error', 'rejected=', d.get('directoryRejected'), 'bundles=', d.get('bundlesDiscovered'))
bad = d.get('directoryRejected') or d.get('bundlesDiscovered', 1) == 0 or d.get('totalFail',0) or d.get('totalError',0)
...
The runtime differences that catch first-time framework contributors — each one has stung a real PR. The full list lives in [Coding Standards](/v4-1-0/contributing/coding-standards/#cross-engine-rules); the ones that bite in *tests* specifically:
...
<CardGrid>
  <LinkCard title="Coding Standards" href="/v4-1-0/contributing/coding-standards/" description="The cross-engine rules in full, plus the pre-push test bar." />
  <LinkCard title="Submitting Pull Requests" href="/v4-1-0/contributing/pull-requests/" description="What CI runs and what reviewers expect." />
  <LinkCard title="Running Tests Locally" href="/v4-1-0/testing/running-tests-locally/" description="Testing your application (not the framework)." />
</CardGrid>
