---
title: "wheels deploy registry setup"
description: Alias for registry login. docker login on every host.
type: reference
sidebar:
  label: "setup"
  order: 1
---

Alias for [`registry login`](/v4-1-0/command-line-tools/commands/deploy/registry/login/). Kept for parity with Kamal's setup verb.

## Synopsis

```txt title="
wheels deploy registry setup [--password=<value>] [--destination=<name>] [--dry-run]
```

Behavior identical to [`registry login`](/v4-1-0/command-line-tools/commands/deploy/registry/login/).

## Example

```bash title="illustrative"
wheels deploy registry setup
```
