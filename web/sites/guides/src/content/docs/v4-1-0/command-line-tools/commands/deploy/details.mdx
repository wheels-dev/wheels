...
1. App `docker ps` filtered by `label=service=<service>` (same as [`wheels deploy app containers`](/v4-1-0/command-line-tools/commands/deploy/app/containers/)).
2. Proxy `docker ps` filtered to the `kamal-proxy` container (same as [`wheels deploy proxy details`](/v4-1-0/command-line-tools/commands/deploy/proxy/details/)).
