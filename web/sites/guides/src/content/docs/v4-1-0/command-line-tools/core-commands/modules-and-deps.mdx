...
- **Deps** are per-project CFML libraries. They live in your project's `lucee.json` file and, once installed, under the project's `dependencies/` directory (the `dependencySettings.installLocation` default — see [Configuration](/v4-1-0/command-line-tools/configuration/)). They can come from git, the Lucee extension registry, ForgeBox, or a local file/JAR.
...
```bash title="install a module from a git URL"
...
```bash title="examples — run in a project with a lucee.json"
wheels deps install
...
```bash {test:cli cmd="wheels deps prune"}
wheels deps prune
```
...
`prune` deletes every cloned git dependency in LuCLI's cache directory, not just unused ones. It does not touch your project's installed dependencies — only the shared clone cache that `deps install` pulls from. Run it when you want to force clean re-clones.
</Aside>
...
Don't confuse LuCLI modules with Wheels packages. Wheels packages (installed into `vendor/<name>/` from standalone repos like `wheels-dev/wheels-sentry` and `wheels-dev/wheels-hotwire`, indexed by the [`wheels-dev/wheels-packages`](https://github.com/wheels-dev/wheels-packages) registry) are optional first-party framework modules that live *inside* a Wheels project. LuCLI modules live *outside* any project, in LuCLI's home directory, and add commands to the CLI. They're fundamentally different mechanisms; this page covers only the CLI-level one.
</Aside>
...
```bash title="install and verify a LuCLI module"
...
```bash title="add a dependency interactively then install"
wheels deps add
wheels deps install
```

`add` walks you through the interactive wizard and writes the new entry into `lucee.json`. `install` then resolves every dependency in that file (including the one you just added) and drops them under the project's install directory (`dependencies/` by default), updating `lucee-lock.json` at the end.

### Prune removed deps

```bash title="prune cache and force reinstall"
wheels deps prune
