...
Same as [`wheels deploy`](/v4-1-0/command-line-tools/commands/deploy/deploy/).
...
- Docker is installed on every host. Run [`wheels deploy bootstrap`](/v4-1-0/command-line-tools/commands/deploy/server/bootstrap/) first if not (the flat alias — the nested `wheels deploy server bootstrap` form is picocli-shadowed).
