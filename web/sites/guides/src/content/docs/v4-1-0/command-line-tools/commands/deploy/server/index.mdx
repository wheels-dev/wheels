...
|------|------------|---------|
| [`exec`](/v4-1-0/command-line-tools/commands/deploy/server/exec/) | `wheels deploy exec <command>` | Run a command on every host (optionally filtered). |
| [`bootstrap`](/v4-1-0/command-line-tools/commands/deploy/server/bootstrap/) | `wheels deploy bootstrap` | Install Docker on a fresh host (idempotent). |
