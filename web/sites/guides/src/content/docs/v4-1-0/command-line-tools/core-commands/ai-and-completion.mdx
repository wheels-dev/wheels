...
Drive Lucee-managed AI endpoints from the CLI. The CLI stores only lightweight local defaults (default endpoint, default model, skill search paths) in `<home>/ai/` — `~/.wheels/ai/` for the `wheels` binary, `~/.lucli/ai/` for standalone LuCLI. The endpoint credentials themselves live in the Lucee server's `CFConfig.json` — `wheels ai config add` writes to that file on your behalf.
...
View or update the CLI-local defaults (stored under `~/.wheels/ai/settings.json`; `~/.lucli/` on standalone LuCLI). These tell other `ai` subcommands which endpoint and model to use when you don't pass `--endpoint` or `--model` explicitly.
...
Global skill paths are stored in `~/.wheels/ai/skill-paths.json` (`~/.lucli/` on standalone LuCLI). Pass `--skills-path` on `ai prompt` or `ai skill list` to add command-local overrides for a single invocation.

### Example

```bash title="illustrative — requires a running Lucee server and API credentials"
...
```bash title="illustrative — shell pipes and redirects not supported in test harness"
...
```bash title="illustrative — shell redirects not supported in test harness"
...
```bash title="illustrative — verify tab completion works after installation"
wheels <TAB>
```

<Aside type="caution" title="Version-list autocomplete requires lucli">
The generated script includes dynamic completion for `--version` values that invokes `lucli versions-list` under the hood. The `wheels` binary is the LuCLI launcher renamed (name-routing selects the wheels module), so this generally works, but if you've aliased `wheels` to a non-LuCLI binary, that portion of completion will fall back to empty.
</Aside>
...
```bash title="illustrative — shell redirect not supported in test harness"
wheels completion md > docs/command-reference.md
```

## Common workflows

```bash title="illustrative — shell features (&&, redirects, subshell) not supported in test harness"
...
<CardGrid>
  ...
  <LinkCard
    ...
    description="The structured AI surface — exposes the CLI's tools (bare function names) to an AI IDE over stdio."
  />
  <LinkCard
