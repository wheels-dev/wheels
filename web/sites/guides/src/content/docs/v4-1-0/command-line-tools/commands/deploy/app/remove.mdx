...
Stop and remove app containers for the given version. Scoped to one version — use [`wheels deploy remove`](/v4-1-0/command-line-tools/commands/deploy/remove/) for a broader teardown.
...
Prefer [`wheels deploy prune`](/v4-1-0/command-line-tools/commands/deploy/prune/) for retention-policy-driven cleanup. `app remove` is surgical — it removes exactly the version you named.
