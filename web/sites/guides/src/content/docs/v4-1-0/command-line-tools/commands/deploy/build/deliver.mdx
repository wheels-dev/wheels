...
Build, push, and pull a new image. Equivalent to `push` followed by `pull`. Skips the rolling traffic switch that [`wheels deploy`](/v4-1-0/command-line-tools/commands/deploy/deploy/) would add.
...
No container is started — use [`wheels deploy`](/v4-1-0/command-line-tools/commands/deploy/deploy/) for a full rollout.
