...
<Aside type="note">
  The framework core doesn't ship i18n primitives. This page documents the pattern you can build in your own app; the first-party [wheels-i18n](https://github.com/wheels-dev/wheels-i18n) package covers the common cases if you'd rather install than build — see [Packages](/v4-1-0/digging-deeper/packages/).
</Aside>

<Aside type="tip">
  **Want a shortcut?** Install the first-party package: `wheels packages add wheels-i18n && wheels reload` (or, without the CLI, extract its release into `vendor/wheels-i18n/` and reload). It ships JSON-file or database-backed translations, parameter interpolation, and pluralization. The pattern below is for when you want full control or a dependency-free implementation.
</Aside>

<Aside type="note">
  You should already know how [middleware](/v4-1-0/core-concepts/middleware-pipeline/) and [views](/v4-1-0/basics/views-layouts-partials/) work. Translation lookup runs in a view helper; locale detection runs in middleware.
</Aside>
...
Make sure locale-scoped routes come before `.wildcard()` — see [how route ordering works](/v4-1-0/core-concepts/how-routing-works/).
