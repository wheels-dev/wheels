...
<Aside type="note">
  You should already know what the DI container is. See [The Dependency Injection Container](/v4-1-0/core-concepts/dependency-injection/) for the concept tour. [Tutorial Part 6b](/v4-1-0/start-here/tutorial/06-authentication/) shows a real registration for `SessionStrategy` — this page unpacks patterns beyond that.
</Aside>
...
`map()` and `bind()` are the same method — `bind()` reads better for interface-style names. Full API tour lives on the [concept page](/v4-1-0/core-concepts/dependency-injection/); the rest of this guide assumes you've registered services this way.
...
This is the pattern [Tutorial Part 6b](/v4-1-0/start-here/tutorial/06-authentication/) uses to wire `authenticator` and `currentUser` into the controllers that need them. It keeps dependencies declared at the top of the file where a reader can see them, rather than scattered across action methods.
...
<CardGrid>
  ...
  <LinkCard
    title="Multi-tenancy"
    href="/v4-1-0/digging-deeper/multi-tenancy/"
