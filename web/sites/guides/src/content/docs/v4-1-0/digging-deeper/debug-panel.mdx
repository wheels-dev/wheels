...
![The Wheels debug bar pinned to the bottom of a browser window: Wheels logo, Posts.index controller.action, timing badge, Params, Development environment, and Tools tabs, with Wheels 4.0.0-SNAPSHOT version on the right](/v4-1-0/digging-deeper/debug-panel-screenshots/01-collapsed-bar.png)
...
![Request panel open showing the key-value table: Route (root), Controller (Posts), Action (index), HTTP Method (GET), URL, Application (blog), Data Source (blog), DB Adapter (SQLiteModel), URL Rewriting (On)](/v4-1-0/digging-deeper/debug-panel-screenshots/02-request-panel.png)
...
![Timing panel showing "Execution Timing — 9ms total" in the header with horizontal bar chart for action and view phases, color-coded and sorted by duration](/v4-1-0/digging-deeper/debug-panel-screenshots/03-timing-panel.png)
...
![Params panel showing the Request Parameters table with Name, Value, Type columns — four rows for status=published, category=tech, author=peter, page=2 — all string type](/v4-1-0/digging-deeper/debug-panel-screenshots/04-params-panel.png)
...
![Environment & Configuration panel showing Application section with Environment (Development with green dot), Wheels Version, CFML Engine (Lucee 7.0.0.395), Host Name; Packages section listing wheels-basecoat; and Plugins (Legacy) "No plugins installed."](/v4-1-0/digging-deeper/debug-panel-screenshots/05-environment-panel.png)
...
![Packages section of the Environment panel — Installed table showing wheels-basecoat 3.0.0-rc.1 with version and description columns](/v4-1-0/digging-deeper/debug-panel-screenshots/05-environment-panel.png)
...
![Tools panel showing the Developer Tools link-card grid: System Info, Routes, API Docs, Guides, Tests, Migrator, Packages, and Plugins](/v4-1-0/digging-deeper/debug-panel-screenshots/06-tools-panel.png)
