...
<Aside type="note">
  You should already know: Wheels controllers, filters, and the `session` scope (see [Controllers and Actions](/v4-1-0/basics/controllers-and-actions/)), plus the end-to-end login flow walked through in [Tutorial Part 6: Authentication](/v4-1-0/start-here/tutorial/06-authentication/). This page assumes you've read both and picks up where Part 6b leaves off.
</Aside>
...
<CardGrid>
  <LinkCard title="Tutorial Part 6: Authentication" href="/v4-1-0/start-here/tutorial/06-authentication/" description="Walks the hand-rolled and built-in flows end-to-end for a blog." />
  <LinkCard title="Authorization Policies" href="/v4-1-0/digging-deeper/authorization-policies/" description="The layer on top: authorize()/can()/policyScope() decide what the authenticated user may do." />
  <LinkCard title="The Dependency Injection Container" href="/v4-1-0/core-concepts/dependency-injection/" description="Why services.cfm, what the three scopes do, and when plain new is still right." />
  <LinkCard title="Controllers and Actions" href="/v4-1-0/basics/controllers-and-actions/" description="Filters, params, and the session scope that session auth builds on." />
</CardGrid>
