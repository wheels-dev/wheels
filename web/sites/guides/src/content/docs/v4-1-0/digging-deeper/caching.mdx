...
1. **Migrate, then reload.** Apply the change through a [migration](/v4-1-0/basics/migrations/) and reload each node with `?reload=true&password=...` — the reload re-reads every model's schema. With more than one app server, reload them one at a time for a zero-downtime rollout.
...
For multi-tenant apps, see [Multi-tenancy](/v4-1-0/digging-deeper/multi-tenancy/) for the tenant-scoped key pattern.
...
Configure edge caching at the deploy layer, not in Wheels. The [Deployment](/v4-1-0/deployment/) guide covers setting `Cache-Control` headers from your controller, letting Kamal's proxy or your CDN honor them, and purging the edge cache when a model saves.
