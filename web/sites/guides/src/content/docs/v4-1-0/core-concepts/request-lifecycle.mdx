...
<Aside type="note">
  This is the conceptual tour. The [Controllers and Actions](/v4-1-0/basics/controllers-and-actions/) page has the hands-on version — how to write each hook.
</Aside>
...
<Aside type="caution">
  Known issue: until [#3061](https://github.com/wheels-dev/wheels/issues/3061) is fixed, any uncaught error — even a stray 404 error page — re-creates the DI container, silently wiping every `config/services.cfm` registration. The next request that relies on an injected service then fails, breaking this guarantee until the application is reloaded.
</Aside>
...
- [MVC in Wheels](/v4-1-0/core-concepts/mvc-in-wheels/) — where each layer lives and what it owns
- [How Routing Works](/v4-1-0/core-concepts/how-routing-works/) — stage 1 in depth
- [Middleware Pipeline](/v4-1-0/core-concepts/middleware-pipeline/) — stage 2 in depth
- [The Dependency Injection Container](/v4-1-0/core-concepts/dependency-injection/) — what `config()` resolves
- [Controllers and Actions](/v4-1-0/basics/controllers-and-actions/) — the hands-on how-to for stages 3-6
