...
Migrate's write commands only connect to a server whose port is explicitly configured for *this* project — the `port` field in `lucee.json` or `PORT` in `.env` — so they can't silently target a sibling app's server. If you see `Wheels.ServerNotRunning`, run `wheels start` in this project's directory first (read-only commands like `migrate info` fall back to probing common ports and print a notice when they do — [#3080](https://github.com/wheels-dev/wheels/issues/3080); the [Database commands reference](/v4-1-0/command-line-tools/wheels-commands/database/) has the full port-resolution rules).
</Aside>
...
The full no-CLI task map lives at [Working Without the CLI](/v4-1-0/start-here/working-without-the-cli/).

## Reconciliation subcommands

Three additional subcommands cover the cases where the tracking table and the on-disk files have drifted — most commonly when several developers share a single dev database. Read more in [Shared development databases](/v4-1-0/basics/shared-development-databases/).
...
2. `t.timestamps()` creates `createdAt`, `updatedAt`, **and** a soft-delete column (`deletedAt`). Don't also add those columns by hand — you'll get duplicate-column errors. See [Models and the ORM](/v4-1-0/basics/models-and-the-orm/) for how Wheels populates them automatically on save.
...
Pass `unique=true` on `addIndex` to create a unique constraint, which is what [`validatesUniquenessOf`](/v4-1-0/basics/validation-and-errors/) should sit on top of to race-proof the check at the database level.
...
<CardGrid>
  <LinkCard title="Models and the ORM" href="/v4-1-0/basics/models-and-the-orm/" description="Models map to the tables migrations create — finders, persistence, validations." />
  <LinkCard title="Associations" href="/v4-1-0/basics/associations/" description="The foreign-key columns that hasMany and belongsTo read from — defined here, declared there." />
  <LinkCard title="Seeding" href="/v4-1-0/basics/seeding/" description="Idempotent seed data via seedOnce() — the right home for reference records." />
  <LinkCard title="Database and Multiple Datasources" href="/v4-1-0/basics/database-and-multiple-datasources/" description="Configuring the connection migrations run against, and targeting a non-default datasource." />
</CardGrid>
