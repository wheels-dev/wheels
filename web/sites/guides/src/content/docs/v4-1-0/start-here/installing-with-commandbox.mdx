...
| Scaffold an app, generators, migrate, test, console | **No** | the [`wheels` CLI](/v4-1-0/start-here/installing/) |
| The packages registry, MCP server, deploy | **No** | the [`wheels` CLI](/v4-1-0/start-here/installing/) |

The short version: **CommandBox gets you the framework and a running server. Reach for the [`wheels` CLI](/v4-1-0/start-here/installing/) for scaffolding** — generators, `wheels migrate`, `wheels test`, `wheels console`, the MCP server, the [packages registry](/v4-1-0/digging-deeper/packages/), and `wheels deploy`. None of those run through CommandBox.

<Aside type="note" title="Two different package systems">
ForgeBox (CommandBox) ships the *framework itself* — `wheels-core` and the app template. The Wheels **packages registry** (`wheels packages ...`) is a separate, `wheels`-CLI-only system for first-party add-ons like `wheels-hotwire` and `wheels-basecoat`. It is **not** on ForgeBox. See [Packages](/v4-1-0/digging-deeper/packages/).
</Aside>
...
Pin a specific engine the same way the [CFML Engines](/v4-1-0/start-here/cfml-engines/) page describes — CommandBox downloads it on first use:
...
CommandBox fetches and serves; it does **not** replace the developer CLI. The following all require the LuCLI-based `wheels` binary (install it from [brew / scoop / apt / yum](/v4-1-0/start-here/installing/) or a [manual JAR](/v4-1-0/command-line-tools/installation/#manual-jar-install)):
...
The CommandBox module published as `wheels-cli` is the pre-4.0 CLI. It does **not** know the 4.0+ command set and is scheduled for removal in Wheels 5 (see [Upgrading 3.x → 4.x](/v4-1-0/upgrading/3x-to-4x/)). Do not install it for new work. The supported developer CLI is the standalone `wheels` binary — see [Installing Wheels](/v4-1-0/start-here/installing/).
</Aside>
...
|---|---|
| `box install wheels-cli` | Install the standalone [`wheels` CLI](/v4-1-0/start-here/installing/) (brew / scoop / apt / yum) |
...
| ForgeBox plugins (`box install ...`, `forgebox install`) | The [`wheels packages`](/v4-1-0/digging-deeper/packages/) registry (CLI-only; not ForgeBox) |
...
- `cfwheels-cli` → the standalone [`wheels` CLI](/v4-1-0/start-here/installing/) (the ForgeBox `cfwheels-cli` module is the pre-rebrand CommandBox CLI; even its renamed successor, the `wheels-cli` slug, is [deprecated](#not-supported-via-commandbox-the-wheels-cli-feature-set))
...
<CardGrid>
  ...
  <LinkCard
    title="CFML Engines"
    href="/v4-1-0/start-here/cfml-engines/"
  ...
  <LinkCard
    ...
    href="/v4-1-0/upgrading/3x-to-4x/"
