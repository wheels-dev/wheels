...
That's enough vocabulary to cover the first hundred specs you'll write. See [Testing](/v4-1-0/testing/) for the full matcher list.
...
The `--filter` flag narrows the run to specs whose path contains the argument — `models`, `controllers`, or a full directory path all work. See [Running Tests Locally](/v4-1-0/testing/running-tests-locally/) for the full flag surface.

<Aside type="note">
  `wheels test` runs against `db/test.sqlite` — a second SQLite file scaffolded alongside `db/development.sqlite` when you ran `wheels new`. The first time the test DB is empty, the framework includes `tests/populate.cfm` to apply your migrations. Subsequent runs reuse the schema until you delete the file. This means the `alice@example.com` account from chapter 6's manual signup walkthrough lives ONLY in `db/development.sqlite` — chapter 7's signup spec gets a clean slate every run. Pass `--no-test-db` if you want app tests to run against the dev DB instead.
</Aside>
...
A Wheels app is a Java web application. You package the app (your code plus `vendor/wheels/` plus your config) and run it on a CFML engine — Lucee or Adobe ColdFusion — on whatever server you choose. Two paths are common: the built-in `wheels deploy` command, or managing your own servers (Docker, a plain VM, IIS — whatever your shop runs).

`wheels deploy` is the CLI's built-in deployer, inspired by Rails' Kamal. You describe your app, your servers, and your domain in `config/deploy.yml`; `wheels deploy setup` provisions the box the first time (installs Docker, pulls your image, wires up a reverse proxy with automatic TLS); `wheels deploy` ships every subsequent change with zero-downtime rolling restarts — no Ruby or extra tools to install. The full walkthrough — image build, secrets, TLS, migrations at boot — starts at [Your First Deploy](/v4-1-0/deployment/first-deploy/). If you'd rather run your own infrastructure, [Docker Deployment](/v4-1-0/deployment/docker-deployment/) and [VM and Bare-metal Deployment](/v4-1-0/deployment/vm-deployment/) cover the self-managed paths — no CLI required.
...
<CardGrid>
  <LinkCard title="Bonus: Style with wheels-basecoat" href="/v4-1-0/start-here/tutorial/08-bonus-basecoat/" description="Optional 30-min chapter. Install your first package and rewrite the post view with shadcn/ui-quality components." />
  <LinkCard title="Core Concepts" href="/v4-1-0/core-concepts/" description="Request lifecycle, MVC, ORM philosophy, DI container, middleware pipeline." />
  <LinkCard title="Digging Deeper" href="/v4-1-0/digging-deeper/" description="Background jobs, email, file uploads, caching, multi-tenancy, packages, SSE." />
  <LinkCard title="Testing" href="/v4-1-0/testing/" description="Full WheelsTest patterns, more browser tests, CI setup, fixtures." />
  <LinkCard title="CLI Reference" href="/v4-1-0/command-line-tools/" description="Every wheels command." />
</CardGrid>
