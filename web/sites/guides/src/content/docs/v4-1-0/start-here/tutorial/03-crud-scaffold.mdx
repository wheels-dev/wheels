...
<Aside type="tip">
  Use `wheels packages add wheels-hotwire` to install from the registry. The `gh repo clone` above is the manual fallback if the registry is unavailable. Part 4 wires Hotwire into the blog for real.
</Aside>
...
The first `curl` smoke-test is fine because it's a GET. Form-driven POST/PUT/DELETE from a browser also works — the layout exposes `<meta name="csrf-token">` and Turbo attaches the token to every write request. But a bare `curl -X POST` from your shell is rejected with `Wheels.InvalidAuthenticityToken` — that's [CSRF protection](/v4-1-0/deployment/security-hardening/#csrf-protection) doing its job, not a bug. Use the browser for the write steps in the tutorial; if you script smoke tests later, scrape the token from a `GET /posts/new` first.
</Aside>
...
In [Part 4: Validations + Turbo Frames](/v4-1-0/start-here/tutorial/04-validations-frames/) you'll add `validatesPresenceOf` to the `Post` model, wrap the form in a `<turbo-frame>`, and watch inline validation errors update without a page reload.

The scaffold's generated controller is a great starting point for [Controllers and Actions](/v4-1-0/basics/controllers-and-actions/). The `_form.cfm` it emits is unpacked in [Views, Layouts, Partials](/v4-1-0/basics/views-layouts-partials/).
