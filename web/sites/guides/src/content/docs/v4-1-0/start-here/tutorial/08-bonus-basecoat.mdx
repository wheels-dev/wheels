...
<Aside type="caution">
  The verb is `add`, not `install`. Typing `wheels packages install <name>` reaches LuCLI's built-in extension installer instead of the Wheels package handler — you'll get a confusing `[INFO] No git or extension dependencies to install` and nothing actually installs. See [`wheels packages` CLI reference](/v4-1-0/digging-deeper/packages/) for why.
</Aside>
...
3. Restart the application so `onApplicationStart` re-fires and `PackageLoader` discovers the new package:
   ...
   `wheels reload` works because an authorized reload stops and restarts the application, re-running `onApplicationStart` — and with it the package loader. A full `wheels stop && wheels start` does the same thing, just more heavily.
...
- **`requires`** — other packages this one needs (with `replaces` and `suggests` as the related fields). Empty for basecoat. A legacy `dependencies` key is ignored by the loader.
...
- [Packages](/v4-1-0/digging-deeper/packages/) — the full lifecycle, the manifest schema, writing your own package, publishing to the registry.
