...
Teams with their own deployment workflow — IIS or nginx in front of Lucee or Adobe ColdFusion, git-based deploys, in-house installers. If you'd rather have a tool scaffold and serve the app for you, use [the wheels CLI](/v4-1-0/start-here/installing/) or [CommandBox](/v4-1-0/start-here/installing-with-commandbox/) instead; all three paths produce structurally identical apps. Not sure which you want? See [Choosing Your Tooling](/v4-1-0/start-here/choosing-your-tooling/).
</Aside>
...
4. **Point your web server at `public/`.** The web root is `public/` and only `public/` — set your IIS site, nginx root, or Apache DocumentRoot there. Everything else (`app/`, `config/`, `vendor/`, `.env`) sits one level up, unreachable from a browser. The mappings in `public/Application.cfc` are relative, so the app runs from any disk location. Your CFML engine (Lucee 5/6/7, Adobe ColdFusion 2018–2025, or BoxLang) must be wired to that site the same way as any other CFML app — see [CFML Engines](/v4-1-0/start-here/cfml-engines/) for engine specifics and [IIS and Windows Deployment](/v4-1-0/deployment/iis-and-windows/) for the full IIS walkthrough.
