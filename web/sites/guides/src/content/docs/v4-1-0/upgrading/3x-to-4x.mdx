...
```cfm title="config/environment.cfm — unreliable pattern"
...
```cfm {test:compile} title="config/environment.cfm — correct pattern"
...
See the [Testing guide](/v4-1-0/testing/) for matchers and the phantom-matcher list.
...
See [Packages](/v4-1-0/digging-deeper/packages/) for manifest fields, per-method mixin overrides via `mixin="model"` or `mixin="none"` annotations, and error isolation. If you need a staged migration, the [`wheels-legacy-adapter`](https://github.com/wheels-dev/wheels-legacy-adapter) package ships deprecation logging and API shims for 3.x patterns.
...
Migrate by running `wheels setup agents` in your project — it writes `.mcp.json` for you — or add the stdio server by hand:

```json title=".mcp.json"
{"mcpServers": {"wheels": {"command": "wheels", "args": ["mcp", "wheels"]}}}
```

Note that bare `wheels mcp` does not print this snippet on the released launcher (it errors with "missing module name"); see the [MCP integration guide](/v4-1-0/command-line-tools/mcp-integration/) for editor-specific variants.
