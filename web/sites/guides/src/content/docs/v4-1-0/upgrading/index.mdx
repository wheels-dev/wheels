...
Wheels doesn't ship on a fixed schedule. Releases go out when the changelog entries warrant one and the test matrix is green. The git tag history is the authoritative record — `git tag --sort=-v:refname` (or the [GitHub releases page](https://github.com/wheels-dev/wheels/releases)) shows the latest release; the 4.0 line went GA 2026-05-12 with `v4.0.0`. Pre-GA builds were tagged in a `v4.0.0-SNAPSHOT+N` series — the `+N` suffix is a continuous build number, not a release.
...
<Aside type="caution">
  Wheels 4.0 deprecates the legacy `plugins/` system in favor of first-party packages distributed as standalone repos (indexed by the [`wheels-dev/wheels-packages`](https://github.com/wheels-dev/wheels-packages) registry) and installed into `vendor/<name>/`. If your 3.x app depends on plugins, read [Packages](/v4-1-0/digging-deeper/packages/) alongside the 3.x→4.x guide — it explains the new activation model.
</Aside>
...
<CardGrid>
  ...
  <LinkCard
    title="2.x to 3.x"
    description="The CFWheels to Wheels rebrand and the foundational directory and config changes in 3.0. Kept here as a pointer for apps still on 2.x. (The WheelsTest BDD migration came later, in 4.0.)"
    href="/v4-1-0/upgrading/2x-to-3x/"
  />
</CardGrid>
