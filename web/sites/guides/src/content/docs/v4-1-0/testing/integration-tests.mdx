...
<Aside type="note">
  You should already know BDD basics and the `TestClient` API. See [Model Tests](/v4-1-0/testing/model-tests/) and [Controller Tests](/v4-1-0/testing/controller-tests/).
</Aside>
...
The `finally` block is what makes this safe: even if an assertion fails midway, the rollback still runs and the next spec sees a clean table. Cross-link to [Fixtures & Test Data](/v4-1-0/testing/fixtures-and-test-data/) for the full fixture-lifecycle pattern — when to use rollback, when to use populate data, when to seed per-spec.
...
<CardGrid>
  ...
  <LinkCard title="View & Form Tests" href="/v4-1-0/testing/view-and-form-tests/" description="Rendering partials, form helper output, and error display." />
  <LinkCard title="Models and the ORM" href="/v4-1-0/basics/models-and-the-orm/" description="Finders, associations, and the persistence APIs your integration tests assert against." />
</CardGrid>
