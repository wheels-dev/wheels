...
<Aside type="note">
  You should already know the request lifecycle. See [The Request Lifecycle](/v4-1-0/core-concepts/request-lifecycle/). For the testing runtime, see [Controller Tests](/v4-1-0/testing/controller-tests/).
</Aside>
...
`withHeader` attaches the request header; the assertion reads the response header the middleware set. See [CORS](/v4-1-0/digging-deeper/cors/) for configuration options and preflight semantics.
...
Reuse one client so the rate-limit key (by default, the client IP) stays stable across the loop. See [Rate Limiting](/v4-1-0/digging-deeper/rate-limiting/) for strategies (`fixedWindow`, `slidingWindow`, `tokenBucket`) and storage options.
...
`assertNotFound()` is sugar for `assertStatus(404)`. See [Route Model Binding](/v4-1-0/digging-deeper/route-model-binding/) for how binding resolves the key into `params.post`.
...
Functional tests cover the server side only — what HTML the server ships on the first request. They can't exercise JavaScript, Turbo Frame/Stream swaps, client-side validation, or form submissions wired to `fetch`. For any flow whose behaviour emerges in the browser after the initial HTML, write a browser test instead. For pure server-rendered flows — classic form POST, redirect, GET — functional tests cover the same ground an order of magnitude faster. See [Browser Tests](/v4-1-0/testing/browser-tests/) for the Playwright-backed DSL.

## Related guides

<CardGrid>
  <LinkCard title="Integration Tests" href="/v4-1-0/testing/integration-tests/" description="Multi-step workflows, cookie-jar session state, and cross-controller assertions on database state." />
  <LinkCard title="Controller Tests" href="/v4-1-0/testing/controller-tests/" description="Single-action response shape, redirects, filters, flash messages, and JSON responses." />
  <LinkCard title="Browser Tests" href="/v4-1-0/testing/browser-tests/" description="Real browser automation via Playwright for JavaScript, Turbo, and UI-driven flows." />
  <LinkCard title="Middleware Pipeline" href="/v4-1-0/core-concepts/middleware-pipeline/" description="How middleware wraps every request — the layer your functional tests exercise before the controller." />
</CardGrid>
