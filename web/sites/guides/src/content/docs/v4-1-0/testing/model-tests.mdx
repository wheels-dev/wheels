...
<Aside type="note">
  You should already know how to define a model. See [Models and the ORM](/v4-1-0/basics/models-and-the-orm/). For the BDD basics — where specs live, how the runner boots — see [Testing](/v4-1-0/testing/).
</Aside>
...
The default transaction mode in the test runner is `none` — writes persist across specs unless you roll them back explicitly. If a spec that passes on its own fails when run with the rest of the suite, look for leftover rows from an earlier spec. The [Fixtures & Test Data](/v4-1-0/testing/fixtures-and-test-data/) guide covers the full setup pattern, `populate.cfm` contents, and strategies for keeping specs independent.
...
<CardGrid>
  <LinkCard title="Models and the ORM" href="/v4-1-0/basics/models-and-the-orm/" description="Defining models, declaring config(), finders and creators." />
  <LinkCard title="Validation and Error Display" href="/v4-1-0/basics/validation-and-errors/" description="The validation helpers you're testing — presence, length, uniqueness, format." />
  <LinkCard title="Associations" href="/v4-1-0/basics/associations/" description="hasMany, belongsTo, dependent=delete, and the association methods your specs exercise." />
  <LinkCard title="Query Builder and Scopes" href="/v4-1-0/basics/query-builder-and-scopes/" description="Named scopes, dynamic scopes, and enum-generated scopes." />
</CardGrid>
