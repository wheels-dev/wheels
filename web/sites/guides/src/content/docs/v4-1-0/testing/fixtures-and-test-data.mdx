...
<Aside type="note">
  You should already know BDD basics. See [Model Tests](/v4-1-0/testing/model-tests/) for the spec shape. This page covers the fixture plumbing every test type relies on.
</Aside>
...
<Aside type="note" title="Isolated test application">
  On apps whose `Application.cfc` includes `vendor/wheels/events/testcontext.cfm` after `config/app.cfm` (`wheels new` and this repo's demo app do), the web runner binds a **separate CFML application name** (`<this.name>_wheelsTest`). Test settings and path swaps apply only to that isolated scope — concurrent normal requests keep the live `application.wheels`. TestClient and browser specs send `X-Wheels-Test-Context` (and a `WHEELS_TEST_CONTEXT` cookie) so fixture HTTP stays on the isolated app. Existing apps that have not applied the include still swap the live scope under the [#3373](https://github.com/wheels-dev/wheels/pull/3373) named lock; add the include to opt in. See [#3374](https://github.com/wheels-dev/wheels/issues/3374).
</Aside>
...
runSql("
    ...
    VALUES ('alice@example.com', 'hash', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),
           ('bob@example.com', 'hash', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
");

runSql("
    INSERT INTO posts (userId, title, body, status, createdAt, updatedAt)
    VALUES (1, 'Welcome', 'First post', 'published', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
");
</cfscript>
```
...
`populate.cfm` runs against whatever database the test run targets. Use `CURRENT_TIMESTAMP` for seeded timestamps — it's ANSI SQL and works on MySQL, PostgreSQL, SQL Server, H2, and SQLite. `NOW()` is not portable: it doesn't exist on SQLite (the inner-loop reference platform) or SQL Server, and no Wheels adapter rewrites it for you. Identity-column syntax diverges more sharply: `AUTOINCREMENT` in SQLite, `SERIAL` in PostgreSQL, `IDENTITY(1,1)` in SQL Server, `AUTO_INCREMENT` in MySQL.
...
- Seed timestamps with `CURRENT_TIMESTAMP` for cross-engine safety (`NOW()` fails on SQLite and SQL Server)
...
<CardGrid>
  <LinkCard title="Model Tests" href="/v4-1-0/testing/model-tests/" description="BDD specs for validations, callbacks, scopes, and associations." />
  <LinkCard title="Controller Tests" href="/v4-1-0/testing/controller-tests/" description="TestClient HTTP patterns — response assertions, redirects, filters, session handling." />
  <LinkCard title="Integration Tests" href="/v4-1-0/testing/integration-tests/" description="Multi-request journeys across your stack — login, create, redirect." />
  <LinkCard title="Running Tests Locally" href="/v4-1-0/testing/running-tests-locally/" description="LuCLI test runner, Docker matrix, common test commands." />
</CardGrid>
