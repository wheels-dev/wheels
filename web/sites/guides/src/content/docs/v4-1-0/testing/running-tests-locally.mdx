...
<Aside type="note">
  You should already know what a spec looks like. See [Model Tests](/v4-1-0/testing/model-tests/) for BDD shape and [Fixtures & Test Data](/v4-1-0/testing/fixtures-and-test-data/) for the populate lifecycle. Looking to run **the framework's own suite** (you're contributing to Wheels itself)? That's a different page: [Running the Framework Test Suite](/v4-1-0/contributing/running-framework-tests/).
</Aside>
...
- **Development only.** The `/wheels/*` surfaces are gated by a development-only allowlist (since 4.0.4, [#2903](https://github.com/wheels-dev/wheels/pull/2903)) — in any other environment they return 404. Run tests where the app runs as `development` (locally, or a CI job configured that way — see [CI Integration](/v4-1-0/testing/ci-integration/)).
- **The datasource is whatever the app is using.** The `<datasource>_test` auto-swap is a CLI convenience; the raw runner exercises the datasource your app is currently configured with. If you drive tests by URL routinely, point your development datasource at a database you're happy to have `tests/populate.cfm` rebuild — or keep populate idempotent (it should be anyway; see [Fixtures & Test Data](/v4-1-0/testing/fixtures-and-test-data/)).
- **Isolated application scope.** With the stock `Application.cfc` snippet (`include` of `vendor/wheels/events/testcontext.cfm` after `config/app.cfm`), `/wheels/app/tests` and `/wheels/core/tests` bind a separate CFML application name. Concurrent browsing of the same server keeps live config. See [Fixtures & Test Data](/v4-1-0/testing/fixtures-and-test-data/).
...
- **Playwright JARs missing** — browser specs skip with `this.browserTestSkipped = true` instead of failing, so the suite stays green. Run `wheels browser setup` once, or see [Browser Tests](/v4-1-0/testing/browser-tests/).
...
<CardGrid>
  <LinkCard title="CI Integration" href="/v4-1-0/testing/ci-integration/" description="Running the suite in GitHub Actions — with or without the CLI." />
  <LinkCard title="Fixtures & Test Data" href="/v4-1-0/testing/fixtures-and-test-data/" description="The populate.cfm lifecycle, test models, and per-spec isolation." />
  <LinkCard title="Browser Tests" href="/v4-1-0/testing/browser-tests/" description="Installing Playwright and running browser specs locally." />
  <LinkCard title="Running the Framework Test Suite" href="/v4-1-0/contributing/running-framework-tests/" description="Contributing to Wheels itself? The core suite, matrix, and gotchas." />
</CardGrid>
