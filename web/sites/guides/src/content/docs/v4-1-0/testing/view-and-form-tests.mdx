...
<Aside type="note">
  You should already know how views and form helpers work. See [Views, Layouts, Partials](/v4-1-0/basics/views-layouts-partials/) and [Forms and Form Helpers](/v4-1-0/basics/forms-and-form-helpers/). For the testing runtime — `describe`, `it`, the `TestClient` API — see [Controller Tests](/v4-1-0/testing/controller-tests/).
</Aside>
...
Object-bound form helpers (see [Forms and Form Helpers](/v4-1-0/basics/forms-and-form-helpers/)) emit **two** identifiers on every field: a dashed `id="post-title"` for DOM labels and CSS, and an underscore `data-auto-id="post_title"` for test selectors. A call like `textField(objectName="post", property="title")` generates both:
...
<CardGrid>
  <LinkCard title="Views, Layouts, Partials" href="/v4-1-0/basics/views-layouts-partials/" description="How views render, where partials live, and how layouts wrap the response body." />
  <LinkCard title="Forms and Form Helpers" href="/v4-1-0/basics/forms-and-form-helpers/" description="Every form helper Wheels ships plus the data-auto-id attribute documented here." />
  <LinkCard title="Controller Tests" href="/v4-1-0/testing/controller-tests/" description="The TestClient API — HTTP verbs, request config, response assertions, and session handling." />
  <LinkCard title="Browser Tests" href="/v4-1-0/testing/browser-tests/" description="Playwright-driven specs for JavaScript-dependent flows — Turbo navigation, Stimulus, real user interaction." />
</CardGrid>
