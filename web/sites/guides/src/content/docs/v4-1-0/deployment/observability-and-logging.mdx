...
- Liveness via the scaffolded `/up` route, plus a deeper readiness check at `/healthz`
...
<Aside type="note">
  You should already be comfortable editing `config/settings.cfm` and installing packages. See [Packages](/v4-1-0/digging-deeper/packages/) for the registry-based distribution and `vendor/` activation model.
</Aside>
...
The component is a few lines of real code: it calls `CreateUUID()`, stashes the value on the request scope, runs the rest of the pipeline, then writes the `X-Request-Id` header on the way out.
...
component implements="wheels.middleware.MiddlewareInterface" output="false" {
    ...
    public string function handle(required struct request, required any next) {
        // The middleware request context carries a `cgi` member with every
        // inbound header under its CGI-style `http_*` name (since 4.0.4 —
        // issue 3074; on earlier versions it only carried
        // {params, route, pathInfo, method}, so read the engine `cgi` scope
        // directly there).
        local.inbound = arguments.request.cgi.http_x_request_id ?: "";
        local.id = Len(local.inbound) ? local.inbound : CreateUUID();

        $writeRequestId(local.id);
    ...
    // Write via a helper with no `request` parameter in scope: inside
    // handle(), the `required struct request` parameter shadows the request
    // scope on Adobe CF, so a bare `request.wheels.requestId = ...` there
    // would write to the passed struct instead of the scope.
...
Apps scaffolded by `wheels new` ship a liveness endpoint already: `app/controllers/Up.cfc` behind a `/up` route, which `wheels deploy`'s proxy probes before flipping traffic and any load balancer can poll. It returns 200 and doubles as a first-request warm-up. If your app predates the scaffold (or you installed manually from the base template), add the route and controller — and when you want a *readiness* check that verifies dependencies rather than just liveness, extend it to ping the database and return JSON:
...
component extends="Controller" {
    ...
    function index() {
        ...
        local.statusCode = local.healthy ? 200 : 503;
...
- **Background jobs** — `Job.cfc` writes failures to the `wheels_jobs` log file (see the retry and permanent-failure `writeLog` calls in `vendor/wheels/Job.cfc`) but does not push them to Sentry. If you want Sentry visibility on job failures, wrap `perform()` in a `try/catch` inside each job and call `application.sentry.captureException()` on the rescue path.
...
<CardGrid>
  <LinkCard
    ...
    href="/v4-1-0/digging-deeper/packages/"
  />
  <LinkCard
