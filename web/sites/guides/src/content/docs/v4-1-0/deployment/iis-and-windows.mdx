...
Plenty of Wheels shops run Windows Server with IIS in front of Adobe ColdFusion or Lucee — no containers, no CLI on the server, deploys from their own repos. This page is that path for Wheels 4. It assumes the app is already on disk per [Manual Installation](/v4-1-0/start-here/manual-installation/) (or copied from your repo); everything here is server wiring.
...
- The app on disk, e.g. `C:\inetpub\wheels-apps\myapp\`, laid out per [Anatomy of a Wheels App](/v4-1-0/start-here/app-structure/)
...
5. **Create `.env`** next to `app\` (one level above the web root) from `env.example`, and set `WHEELS_ENV` and `WHEELS_RELOAD_PASSWORD`. Reload later with `?reload=true&password=…` — no server access needed. See [Working Without the CLI](/v4-1-0/start-here/working-without-the-cli/).
...
- **SQLite** works for development on Windows (`db\` folder), but for IIS production you'll typically run SQL Server or MySQL — see [Database and Multiple Datasources](/v4-1-0/basics/database-and-multiple-datasources/).
