...
The `${VAR}` lookup chain, the uppercase-only token rule, and the secrets-path caveat ([#3084](https://github.com/wheels-dev/wheels/issues/3084)) are documented once, in [the config reference](/v4-1-0/deployment/config-reference/#variable-interpolation).
...
- `retain_containers` — pass `--keep=<n>` to [`wheels deploy prune`](/v4-1-0/command-line-tools/commands/deploy/prune/) instead (default keeps the 5 most recent)
- `minimum_version`, `asset_path`, `require_destination`, `allow_empty_roles`, `run_directory`, `readiness_delay`

See the [Rejected Kamal keys](/v4-1-0/deployment/config-reference/#rejected-kamal-keys) table in the config reference for what each key does in Kamal and the `wheels deploy` behavior you get instead.
...
4. **Install the Wheels CLI on every control machine that runs deploys.** See [installing the CLI](/v4-1-0/command-line-tools/installation/).
