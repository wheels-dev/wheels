...
<Aside type="note">
  You should already know how Wheels reads its configuration — see [Environments and Configuration](/v4-1-0/core-concepts/environments-and-configuration/). You need root (or sudo) on a Linux host and a domain pointed at its public IP.
</Aside>
...
Wheels reads these at boot via `env("VAR_NAME")`. See [Environments and Configuration](/v4-1-0/core-concepts/environments-and-configuration/#secrets-handling) for where that lookup happens.
...
- `EnvironmentFile=` loads the `.env` into the service's environment. `WHEELS_ENV=production` is what flips Wheels into [production mode](/v4-1-0/core-concepts/environments-and-configuration/#environment-detection).
...
The `@rewrite` block is what makes `URLRewriting="On"` work — without it every generated link 404s. [URL Rewriting](/v4-1-0/deployment/url-rewriting/) covers the same rule for Apache, IIS, and Tuckey, plus the `Partial` fallback if you'd rather skip server rewrites entirely.
...
<Aside type="note">
  `X-Forwarded-Proto` is what tells Wheels the request came in over HTTPS even though Lucee itself is listening on plain HTTP inside the box — but the header alone isn't enough. Wheels ignores proxy headers unless you opt in with `set(trustProxyHeaders=true)` in `config/settings.cfm`; without that setting, `isSecure()`-style checks and protocol-aware redirects misfire even with the `proxy_set_header` line above in place. See [Trusted proxies and client IP](/v4-1-0/deployment/security-hardening/#trusted-proxies-and-client-ip).
</Aside>
...
4. **Decide how migrations run.** Nothing on this VM has the `wheels` CLI, and the staged release has no running app for the CLI to talk to anyway — so pick one of the in-framework paths:
   ...
   - **After the flip, on demand:** trigger the migrator programmatically through a protected code path — `application.wheels.migrator.migrateToLatest()` — from an authenticated admin action or deploy hook. See [Running migrations without the wheels CLI](/v4-1-0/basics/migrations/#running-migrations-without-the-wheels-cli).
