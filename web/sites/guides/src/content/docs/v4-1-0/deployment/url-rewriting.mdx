...
The full production server block (TLS, upstream keepalive, logging) lives in [VM and Bare-metal Deployment](/v4-1-0/deployment/vm-deployment/#nginx-reverse-proxy).
...
The IIS URL Rewrite Module translation — including the connector setup around it — lives on [IIS and Windows Deployment](/v4-1-0/deployment/iis-and-windows/); the rule itself is the same shape: match everything, negate real files and directories, rewrite to `index.cfm/{R:1}`.
