...
<Aside type="note">
  You should already know [Environments and Configuration](/v4-1-0/core-concepts/environments-and-configuration/). This guide is task-oriented — it assumes you've shipped at least one Wheels app in development.
</Aside>
...
- **Migrations are not auto-run.** `autoMigrateDatabase` is `false` in every environment. Run migrations explicitly from your deploy flow — `wheels migrate latest`, or [one of the no-CLI paths](/v4-1-0/basics/migrations/#running-migrations-without-the-wheels-cli) — or opt in with `set(autoMigrateDatabase=true)`.
...
<CardGrid>
  <LinkCard
    ...
    href="/v4-1-0/core-concepts/environments-and-configuration/"
  />
  <LinkCard
    title="Caching"
