...
Empty inputs to `WHERE IN` aren't an exotic edge case. They're what you get whenever the values come from another query, a form filter, or any computation that might return zero results. The Rails community converged on this pattern in 2016, Sequel matches it, Django matches it, Laravel Eloquent matches it: an empty `IN` short-circuits to `1=0` (no rows), and an empty `NOT IN` short-circuits to `1=1` (every row). It's what the SQL spec implies and what every other framework's users expect.

Wheels now does the same. `whereIn("id", [])` appends `1 = 0` to the WHERE chain and matches no rows; `whereNotIn("id", [])` appends `1 = 1` and matches every row. The clauses compose cleanly with everything else — `.where("status", "active").whereIn("id", []).count()` still returns 0 because the AND-combined chain can't satisfy `1=0`. Four new specs in `queryBuilderSpec.cfc` lock the behaviour in. The reference table in the query-builder guide notes the short-circuit so you don't have to read the source to confirm it.
