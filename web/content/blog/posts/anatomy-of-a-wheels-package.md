...
A package can override the auto-derivation by setting `mapping` explicitly in the manifest — useful when the camelCase form clashes with something else in your app or when the derivation produces a name you don't like. The override must match the CFML identifier regex `[A-Za-z_][A-Za-z0-9_]*`; an invalid value or an empty string fails the package at load time. Two packages that compute (or declare) the same alias both fail with `Duplicate package mapping alias`, naming both claimants. The first claimant keeps its alias; the second is rolled back — its mixins are never partially applied, and you fix it by setting a unique `mapping` in the second package's manifest.
