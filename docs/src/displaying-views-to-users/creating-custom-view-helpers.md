...
### The templates/base/src/app/views/helpers.cfm File
...
```javascript
templates/base/src/app/views/users/helpers.cfm
```
...
Helper functions, together with the use of [Partials](https://wheels.dev/3.0.0/guides/displaying-views-to-users/partials), gives you a way to keep your code nice and DRY, but there are a few things to keep in mind as you work with them.

The `helpers.cfm` files are only meant to be used for views, hence the placement in the _**templates/base/src/app/views**_ folder.

If you need to share non-view functionality across controllers, then those should be placed in the parent controller file, i.e. `templates/base/src/app/controllers/Controller.cfc`. If you need helper type functionality within a **single** controller file, you can just add it as a function in that controller and make it private so that it can't be called as an action (and as a reminder to yourself of its general purpose as well).

The same applies to reusable model functionality: use the parent file, `templates/base/src/app/models/Model.cfc`. Private functions within your children models work well here, just like with controllers.

If you need to share a function globally across your entire application, regardless of which MVC layer that will be accessing it, then you can place it in the `templates/base/src/app/events/functions.cfm` file.
