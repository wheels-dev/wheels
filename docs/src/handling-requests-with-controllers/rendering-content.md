...
response functions ( [renderView()](https://wheels.dev/api/v3.0.0/controller.renderview.html), [sendFile()](https://wheels.dev/api/v3.0.0/controller.sendfile.html) etc) then Wheels will\
...
chapters about [Redirecting Users](https://wheels.dev/3.0.0/guides/handling-requests-with-controllers/redirecting-users) and [Sending Files](https://wheels.dev/3.0.0/guides/handling-requests-with-controllers/sending-files) cover the other two\
response methods.

### Rendering a Page

This is the most common way of responding to the user. It's done with the\
[renderView()](https://wheels.dev/api/v3.0.0/controller.renderview.html) function, but most often you probably won't call it\
...
it's very important to keep in mind that [renderView()](https://wheels.dev/api/v3.0.0/controller.renderview.html) will\
**not run the code** for the controller's action—all it does is process the view\
page for it.

You can also call [renderView()](https://wheels.dev/api/v3.0.0/controller.renderview.html) explicitly if you wish to cache the response\
...
Refer to the [Pages](https://wheels.dev/3.0.0/guides/displaying-views-to-users/pages) chapter for more details about rendering content. More\
specifically, that chapter describes where to place those files and what goes in\
them.

### Rendering a Partial

This is done with the [renderPartial()](https://wheels.dev/api/v3.0.0/controller.renderpartial.html) function. It's most often used with\
...
the [renderNothing()](https://wheels.dev/api/v3.0.0/controller.rendernothing.html) function to tell Wheels to just render an empty page to\
the browser.

### Rendering Text

This is done with the [renderText()](https://wheels.dev/api/v3.0.0/controller.rendertext.html) function. It just returns the text you\
...
handy. It's available on both [renderView()](https://wheels.dev/api/v3.0.0/controller.renderView.html) and [renderPartial()](https://wheels.dev/api/v3.0.0/controller.renderpartial.html).\
...
the default cache time set in `templates/base/src/config/settings.cfm`) or `cache=x` where `x` is\
...
called [Using Layouts](https://wheels.dev/3.0.0/guides/displaying-views-to-users/layouts).
