...
**Model policy:** judging and coding stages run on Opus; janitorial stages (auto-close, write-docs, update-docs) run on Sonnet. Exact model IDs are pinned by each workflow's `--model` flag in `.github/workflows/bot-*.yml`.

**Retired (2026-06-11):** the Reviewer A / Reviewer B convergence loop (`bot-review-b.yml`, `/review-the-review`, `/respond-to-critique`) and the `converged-*` auto-fire chain into address-review. The loop was expensive and flaky, B's marginal catch rate no longer justified a second model pass, and the auto-fire push chain landed a broken spec on a PR (#3005). The loop's deadlock-breaker (`bot-advisor.yml`, `/advise-on-deadlock`) was removed on 2026-09-22; historical `wheels-bot:advisor:` markers are no longer produced or consumed.
