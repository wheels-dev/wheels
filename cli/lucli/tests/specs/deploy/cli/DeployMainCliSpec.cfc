    ...
    function run() {
        describe("DeployMainCli", () => {
            ...
            it("version() reports the pinned Kamal version", () => {
                ...
                var dc = new modules.wheels.services.deploy.cli.DeployMainCli(fake);
            ...
            it("dryRunOutput() includes pre-deploy and post-deploy hook markers when hooks exist", () => {
                ...
                var localCli = new modules.wheels.services.deploy.cli.DeployMainCli(
            ...
            it("init_stub writes config/deploy.yml and .kamal/secrets to the target cwd", () => {
                ...
                var fake = new modules.wheels.services.deploy.lib.FakeSshPool();
                var localCli = new modules.wheels.services.deploy.cli.DeployMainCli(fake);
            ...
            it("docs without section prints the TOC", () => {
                var dc = new modules.wheels.services.deploy.cli.DeployMainCli(
                    new modules.wheels.services.deploy.lib.FakeSshPool()
                );
                var out = dc.docs({});
