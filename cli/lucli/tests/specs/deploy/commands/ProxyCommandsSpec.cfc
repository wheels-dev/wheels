    ...
    function run() {
        describe("ProxyCommands", () => {
            ...
            it("boot() runs the pinned kamal-proxy image", () => {
                var cmd = new modules.wheels.services.deploy.commands.ProxyCommands(variables.cfg).boot();
            ...
            it("deploy() produces the hand-off to kamal-proxy CLI", () => {
                var cmd = new modules.wheels.services.deploy.commands.ProxyCommands(variables.cfg)
            ...
            it("remove() stops and removes the proxy container", () => {
                var cmd = new modules.wheels.services.deploy.commands.ProxyCommands(variables.cfg).remove();
            ...
            it("details() filters ps to the proxy container", () => {
                var cmd = new modules.wheels.services.deploy.commands.ProxyCommands(variables.cfg).details();
            ...
            it("reboot() chains remove + boot", () => {
                var cmd = new modules.wheels.services.deploy.commands.ProxyCommands(variables.cfg).reboot();
                expect(cmd).toInclude("docker stop kamal-proxy");
                expect(cmd).toInclude("docker rm kamal-proxy");
                expect(cmd).toInclude("docker run");
            });
            ...
            it("start() starts the kamal-proxy container", () => {
                var cmd = new modules.wheels.services.deploy.commands.ProxyCommands(variables.cfg).start();
                expect(cmd).toBe("docker start kamal-proxy");
            });

            it("stop() stops the kamal-proxy container", () => {
                var cmd = new modules.wheels.services.deploy.commands.ProxyCommands(variables.cfg).stop();
                expect(cmd).toBe("docker stop kamal-proxy");
            });

            it("restart() restarts the kamal-proxy container", () => {
                var cmd = new modules.wheels.services.deploy.commands.ProxyCommands(variables.cfg).restart();
