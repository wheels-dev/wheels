 ...
 *
 * Examples:
 * wheels g code auth-filter
 * wheels g code --list
 * wheels g code --category=model
 */
component aliases="wheels g code" extends="../base" {

    property name="detailOutput" inject="DetailOutputService@wheels-cli";

    /**
     * @pattern.hint Code pattern name
     * @list.hint Show all available code snippets
     ...
     * @customize.hint Create custom code snippet
     * @create.hint Create new code template
    ...
    private function listSnippets(string category = "") {
        ...
        detailOutput.header("Available Code Snippets");
    ...
    private function createCustomSnippet(required string name) {
        detailOutput.header("Creating Custom Code Snippet");

        var snippetDir = getCWD() & "/app/snippets/" & arguments.name;

        if (directoryExists(snippetDir)) {
            detailOutput.error("Code snippet '#arguments.name#' already exists");
            setExitCode(1);
            return;
        }
        ...
        var templateContent = '// Custom code snippet: #arguments.name#' & chr(10) &
                             '// Add your code here';

        fileWrite(snippetDir & "/template.txt", templateContent);

        detailOutput.create("Created: #snippetDir#");
        detailOutput.success("Custom code snippet '#arguments.name#' created successfully!");

        detailOutput.nextSteps([
            "Edit: #snippetDir#/template.txt",
            "Use: wheels g code #arguments.name#"
        ]);
    }
