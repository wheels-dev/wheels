...
component extends="../base" {
	...
	*/
	function run(
		...
		try {
			...
			print.blinkingRed("Please wait...");
			print.line();
			
			local.testResults = runTestsWithCoverage(local.config);
			
			// Clear the blinking message
			print.line();
			...
			if (arguments.failOnLow && arguments.threshold > 0) {
				if (local.coverage.percentage < arguments.threshold) {
					print.line();
					print.redBoldLine("================================================================================");
					print.redLine("COVERAGE FAILED: #numberFormat(local.coverage.percentage, '0.0')#% is below threshold of #arguments.threshold#%");
					print.redBoldLine("================================================================================");
				}
			}
	...
	*/
	private struct function prepareCoverageConfig(required struct args) {
		...
		try {
			...
			config.host = local.serverInfo.host;
			config.port = local.serverInfo.port;
		} catch (any e) {
			config.host = "localhost";
			config.port = "8080";
			print.yellowLine("Warning: Using default server (localhost:8080)");
		}
		
		// Build test URL
		local.baseUrl = "http://#config.host#:#config.port#/";
		
		if (config.type == "app") {
			config.testUrl = baseUrl & "?controller=wheels.public&action=testbox&view=runner";
		} else if (config.type == "core") {
			config.testUrl = baseUrl & "?controller=wheels.public&action=tests_testbox&view=runner";
		} else {
			// Plugin tests
			config.testUrl = baseUrl & "?controller=tests&action=runner&view=runner&type=#config.type#";
		}
		
		// Add parameters
		config.testUrl &= "&format=json";
		config.testUrl &= "&coverage=true";
		
		if (config.reload) {
			config.testUrl &= "&reload=true";
		}
	...
	*/
	private struct function runTestsWithCoverage(required struct config) {
		if (config.debug) {
			print.line("Test URL: #config.testUrl#");
		}
		
		try {
			local.httpService = new http();
			local.httpService.setUrl(config.testUrl);
			local.httpService.setMethod("GET");
			local.httpService.setTimeout(300);
			
			local.result = local.httpService.send().getPrefix();
			
			if (!isJson(local.result.fileContent)) {
				throw(message="Invalid response from test runner", detail=left(local.result.fileContent, 500));
			}
			
			return deserializeJson(local.result.fileContent);
			
		} catch (any e) {
			throw(message="Failed to execute tests", detail=e.message);
		}
	}
	...
	private struct function processCoverageResults(required struct testResults, required struct config) {
		...
		if (structKeyExists(testResults, "coverage")) {
			...
			if (coverage.enabled && structKeyExists(testResults.coverage, "data") && !structIsEmpty(testResults.coverage.data)) {
				...
				print.greenLine("Coverage data available - processing actual metrics...");
				...
				print.yellowLine("Coverage not enabled - estimating from test results...");
				coverage = estimateCoverageFromTests(testResults, coverage);
			}
		} else {
			// No coverage key at all
			print.yellowLine("No coverage data available - estimating from test results...");
			coverage = estimateCoverageFromTests(testResults, coverage);
		}
	...
	*/
	private void function displayConsoleSummary(required struct coverage, required struct config, required struct testResults) {
		...
		if (coverage.enabled) {
			...
			print.line("  1. Ensure TestBox 4.0+ is installed");
...
<coverage version="1" timestamp="#getTickCount()#">
	...
	*/
	private void function generateBadge(required struct coverage, required struct config) {
		...
		local.color = getCoverageColor(coverage.percentage);
		
		local.svg = '<svg xmlns="http://www.w3.org/2000/svg" width="104" height="20">
	...
	*/
	private void function compareWithBaseline(required struct coverage, required struct config) {
		...
		if (!fileExists(baselineFile)) {
			print.line();
			print.yellowLine("No baseline found for comparison");
	...
	*/
	private string function getCoverageColor(required numeric percentage) {
