...
//=====================================================================
	...
	*/
    ...
    /**
     * Execute HTTP request to test endpoint
     */
    function executeTestRequest(
        required string url,
        numeric timeout = 300,
        boolean debug = false
    ) {
        if (arguments.debug) {
            print.line("Test URL: #arguments.url#");
        }
        
        try {
            local.httpService = new http();
            local.httpService.setUrl(arguments.url);
            local.httpService.setMethod("GET");
            local.httpService.setTimeout(arguments.timeout);
            
            local.result = local.httpService.send().getPrefix();
            
            return {
                success = true,
                content = local.result.fileContent,
                statusCode = local.result.statusCode ?: 200
            };
    ...
    function buildTestUrl(
        ...
        if (arguments.type == "app") {
            local.url = local.baseUrl & "?controller=wheels.public&action=testbox&view=runner";
        } else if (arguments.type == "core") {
            local.url = local.baseUrl & "?controller=wheels.public&action=tests_testbox&view=runner";
        } else {
            local.url = local.baseUrl & "?controller=tests&action=runner&view=runner&type=#arguments.type#";
        }
        
        // Add additional parameters
        for (local.key in arguments.params) {
            if (len(arguments.params[local.key])) {
                local.url &= "&#local.key#=#arguments.params[local.key]#";
            }
        }
    ...
    /**
     * Parse test response based on content type
     */
    function parseTestResponse(required string content) {
        if (isJson(arguments.content)) {
            return deserializeJson(arguments.content);
        }
        
        // Parse HTML response for basic metrics
        return {
            totalSpecs = 0,
            totalPass = 0,
            totalFail = 0,
    ...
    function resolvePath(required string path) {
        if (structKeyExists(variables, "fileSystemUtil")) {
     ...
     */
    function getCoverageColor(required numeric percentage) {
        if (arguments.percentage >= 80) return "##28a745";
        if (arguments.percentage >= 60) return "##ffc107";
        return "##dc3545";
    }
    ...
    function formatDuration(required numeric milliseconds) {
        return numberFormat(arguments.milliseconds / 1000, "0.00");
    }
