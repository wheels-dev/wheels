...
component implements="wheels.middleware.MiddlewareInterface" output="false" {
	 ...
	 * Dispatches dialect-aware DDL so storage="database" actually works on every
	 * engine the framework supports — not just MySQL. Prior to this dispatch,
	 * the single CREATE TABLE used AUTO_INCREMENT + inline INDEX clauses
	 * (MySQL-only) and threw silently on SQLite/PostgreSQL/SQL Server/Oracle/H2;
	 * the swallowed error left the table missing, every $dbIncrement INSERT and
	 * UPDATE failed, and $handleError fail-closed-blocked every request.
	 */
	private void function $ensureTable() {
		...
		var dbType = $detectDatabaseType();

		try {
			QueryExecute($buildCreateTableSql(dbType));
		} catch (any e) {
			// Table may already exist — that's fine; INSERT/UPDATE will report any
			// genuine schema problem on the next call.
		}

		// Indexes are created in separate statements because most dialects don't
		// accept INDEX clauses inside CREATE TABLE. Each runs in its own try/catch
		// so "already exists" errors on re-run are no-ops.
		var indexSqls = $buildCreateIndexSqls(dbType);
		for (var idxSql in indexSqls) {
			try {
				QueryExecute(idxSql);
			} catch (any e) {
	...
	/**
	 * Detect the underlying database engine via cfdbinfo against the default
	 * Wheels datasource. Returns "mysql", "postgresql", "sqlite", "sqlserver",
	 * "oracle", "h2", or "default" when detection is unavailable. Mirrors the
	 * pattern in vendor/wheels/Job.cfc::$detectDatabaseType().
	 */
	private string function $detectDatabaseType() {
		try {
			...
			if (StructKeyExists(application, "wheels") && StructKeyExists(application.wheels, "dataSourceName")) {
				ds = application.wheels.dataSourceName;
			}
			if (Len(ds)) {
				cfdbinfo(type = "version", datasource = ds, name = "local.info");
				var product = local.info.database_productname;
				if (FindNoCase("oracle", product)) return "oracle";
				if (FindNoCase("postgre", product)) return "postgresql";
				if (FindNoCase("h2", product)) return "h2";
				if (FindNoCase("mysql", product) || FindNoCase("mariadb", product)) return "mysql";
				if (FindNoCase("sql server", product)) return "sqlserver";
				if (FindNoCase("sqlite", product)) return "sqlite";
			}
		} catch (any e) {
			// cfdbinfo not available or datasource not configured — fall through.
		}
		return "default";
	}

	/**
	 * Dialect-aware CREATE TABLE SQL for wheels_rate_limits.
	 * Per-dialect notes:
	 *   - mysql       AUTO_INCREMENT + TIMESTAMP NULL
	 *   - postgresql  SERIAL + TIMESTAMP NULL
	 *   - sqlite      INTEGER PRIMARY KEY AUTOINCREMENT (note: lowercase, no underscore)
	 *   - h2          AUTO_INCREMENT (H2 accepts the MySQL-style spelling)
	 *   - sqlserver   IDENTITY(1,1) + DATETIME; legacy SQL Server lacks IF NOT EXISTS,
	 *                 so a sys.tables guard wraps the CREATE.
	 *   - oracle      GENERATED BY DEFAULT AS IDENTITY (12c+); VARCHAR2; TIMESTAMP NULL
	 *   - default     standards-ish fallback with a manual id column — best-effort
	 *                 for engines the framework hasn't been explicitly verified on.
	 */
	private string function $buildCreateTableSql(required string dbType) {
		switch (arguments.dbType) {
			case "mysql":
				return "CREATE TABLE IF NOT EXISTS wheels_rate_limits (
			...
			case "sqlserver":
				return "IF NOT EXISTS (SELECT 1 FROM sys.tables WHERE name = 'wheels_rate_limits')
			...
			case "oracle":
				return "CREATE TABLE wheels_rate_limits (
					id NUMBER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
					store_key VARCHAR2(255) NOT NULL,
	...
	/**
	 * Dialect-aware CREATE INDEX statements for wheels_rate_limits.
	 * Engines that accept "IF NOT EXISTS" on indexes use it; others rely on the
	 * caller's try/catch to absorb "already exists" errors on re-run.
	 */
	private array function $buildCreateIndexSqls(required string dbType) {
		var indexes = [];
		switch (arguments.dbType) {
			case "mysql":
			case "postgresql":
			case "sqlite":
			case "h2":
				ArrayAppend(indexes, "CREATE INDEX IF NOT EXISTS idx_wheels_rate_limits_store_key ON wheels_rate_limits (store_key)");
				ArrayAppend(indexes, "CREATE INDEX IF NOT EXISTS idx_wheels_rate_limits_expires_at ON wheels_rate_limits (expires_at)");
				break;
			default:
				// SQL Server / Oracle / unknown — no IF NOT EXISTS; the caller's
				// try/catch handles "already exists" on re-run.
				ArrayAppend(indexes, "CREATE INDEX idx_wheels_rate_limits_store_key ON wheels_rate_limits (store_key)");
				ArrayAppend(indexes, "CREATE INDEX idx_wheels_rate_limits_expires_at ON wheels_rate_limits (expires_at)");
				break;
		}
		return indexes;
	}
