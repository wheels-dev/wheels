...
<cfelseif local.db IS "postgresql">
	<cfquery name="local.versionCheck" datasource="#application.wheels.dataSourceName#">
		SELECT version() AS versionString
	</cfquery>
	<cfif FindNoCase("CockroachDB", local.versionCheck.versionString)>
		<cfset local.identityColumnType = "INT8 GENERATED BY DEFAULT AS IDENTITY">
		<cfset local.charType = "varchar(9)">
	<cfelse>
