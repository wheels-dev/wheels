 ...
 *
 * The SAME bug class lived in the convergence-loop implementer, which was out
 * of scope for ##2865 because it fires on a different trigger path (not the
 ...
 *   - bot-address-review.yml gains a resolve step that captures
 ...
 *   - address-review.md takes `<head-sha>` and emits its markers from it instead of from `gh pr view --json headRefOid`.
 *
			...
			describe("bot-address-review.yml", () => {
				...
				it("resolves the head SHA once at the workflow level", () => {
					...
					expect(reFindNoCase("headRefOid", content) > 0).toBeTrue(
						...
						& "SHA can be threaded into the prompt (issue ##2848 / PR ##2865)."
					);
				});
