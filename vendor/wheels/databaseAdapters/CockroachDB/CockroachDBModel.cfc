component extends="wheels.databaseAdapters.PostgreSQL.PostgreSQLModel" output=false {
	...
	/**
	 * Override PostgreSQL's $identitySelect to handle CockroachDB's unique_rowid().
	 *
	 * CockroachDB tables may use either SERIAL (which creates a sequence) or
	 * INT DEFAULT unique_rowid() (no sequence). For SERIAL columns,
	 * pg_get_serial_sequence()/currval() works. For unique_rowid() columns,
	 * we fall back to the JDBC driver's generatedKey result.
	 */
	public any function $identitySelect(
		...
		if (Left(local.sql, 11) == "INSERT INTO" && !StructKeyExists(arguments.result, $generatedKey())) {
			...
			if (!ListFindNoCase(local.columnList, ListFirst(arguments.primaryKey))) {
				...
				// Try pg_get_serial_sequence first (works for SERIAL columns)
				try {
					...
					if (
						...
						local.rv[$generatedKey()] = local.idQuery.lastId;
						return local.rv;
					}
				} catch (any e) {
					// pg_get_serial_sequence not available or failed
				}

				// Fallback for unique_rowid() columns: check JDBC-provided generatedKey.
				// The PG JDBC driver uses RETURNING * which returns all columns as a
				// comma-separated string (e.g. "123456,Per,Djurner"). Extract only the
				// first value (the generated primary key).
