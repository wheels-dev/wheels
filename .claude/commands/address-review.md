...
This is a *coding* stage like propose-fix — broad allowlist with the test
runner. Mirrors propose-fix's safety patterns.
...
- `<head-sha>` — the PR head SHA at the start of this run, captured once by
  ...
  the SHA yourself — re-deriving it mid-session races with new pushes. This
  governs only the marker SHA; you still use `gh pr view` normally to read
  reviews, comments, and the head ref name.

## Steps

1. **Idempotency + outer-loop cap.** Read PR comments
