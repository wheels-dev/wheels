...
13. **`cfabort;` in script context is Lucee-only — use `abort;`.** Adobe CF (all versions — 2018/2021/2023/2025) does not recognize `cfabort` as a script keyword and resolves it as a variable reference instead, throwing `Variable CFABORT is undefined` at runtime. The portable form is the script keyword `abort;`; the parenthesized tag form `cfabort()` also works. Fixed in [#3029](https://github.com/wheels-dev/wheels/issues/3029).
