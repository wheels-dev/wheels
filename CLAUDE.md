...
14. **Bare `$helper()` calls in non-mixin event CFCs are out of scope on Lucee.** `vendor/wheels/events/*.cfc` files declared as bare `component {` (no `extends`) are NOT processed by the `$integrateComponents()` mixin pass that model/controller objects receive. A bare call to a `wheels.Global` helper (e.g. `$location()`) in such a file resolves against the component's own scope, which does not carry the helper surface — Lucee throws `No matching function [$LOCATION] found` and the application cold start aborts. Use `application.wo.$helper(...)` for every `wheels.Global` call in a bare event CFC, matching the other helper calls already in those files. Structural guard: `vendor/wheels/tests/specs/security/EventHelperScopeGuardSpec.cfc` fails the suite if any bare event CFC calls an out-of-scope helper ([#3054](https://github.com/wheels-dev/wheels/issues/3054)).
